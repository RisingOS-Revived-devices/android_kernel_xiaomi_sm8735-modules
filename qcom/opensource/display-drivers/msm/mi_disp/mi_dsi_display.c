...
int mi_dsi_display_set_mipi_rw(void *display, char *buf)
{
	...
	if (buffer[0] <= 0x01) {
		...
		ctl.rx_state = MI_DSI_CMD_LP_STATE;
		DISP_INFO(
			"transfer speed not set, default use LPM. buffer[0] = 0x%02x\n",
			buffer[0]);
	}
...
int mi_dsi_display_read_panel_build_id(struct dsi_display *display)
{
	...
	if (strlen(build_id_info_str)) {
		...
		rc = mi_dsi_display_cmd_read_locked(
			display, cmd, &config->build_id, config->id_cmds_rlen);
		if (rc <= 0) {
			DISP_ERROR(
				"[DSI] Display command receive failed, rc=%d\n",
				rc);
			return rc;
		}
	}
						...
						size_t size)
{
	...
	if (strlen(build_id_info_str)) {
		...
		real_build_id = mi_get_panel_build_id(
			panel->mi_cfg.mi_panel_id, panel->id_config.build_id);
...
static int mi_dsi_display_re_read_wp_info(struct dsi_display *display)
{
	...
	rc = mi_dsi_display_cmd_read_panel_locked(
		display, cmd, config->return_buf, config->wp_cmds_rlen);
	if (rc <= 0)
...
int mi_dsi_display_get_fps(void *display, struct disp_fps_info *fps_info)
{
	...
	if (cur_mode) {
		...
		memcpy(&fps_info->mode, &cur_mode->mi_timing,
...
int mi_dsi_display_set_fp_unlock_state(void *display, u32 fp_unlock_value)
{
	struct dsi_display *dsi_display = (struct dsi_display *)display;
						...
						char *buf, size_t size)
{
	...
	memset(config->return_buf, 0x0, sizeof(*config->return_buf));
	rc = mi_dsi_display_cmd_read_panel_locked(
		display, cmd, config->return_buf, config->cell_id_cmds_rlen);
...
ssize_t mi_dsi_display_read_cell_id(void *display, char *buf, size_t size)
{
	...
	if (!strcmp(dsi_display->display_type, "primary")) {
		if (!strlen(cell_id_info_str)) {
			mi_dsi_acquire_wakelock(dsi_display->panel);
			ret = mi_dsi_display_read_ddic_cell_id(
				dsi_display, cell_id_read_buffer, size);
			mi_dsi_release_wakelock(dsi_display->panel);
			ret = snprintf(buf, size, "%s", cell_id_read_buffer);
			DISP_INFO("primary panel cell id is %s", buf);
		} else {
			ret = snprintf(buf, size, "%s\n", cell_id_info_str);
			DISP_INFO("primary panel cell id is %s",
				  cell_id_info_str);
		}
	}
	if (!strcmp(dsi_display->display_type, "secondary")) {
		if (!strlen(sec_cell_id_info_str)) {
			...
			DISP_INFO("secondary panel cell id is %s",
				  sec_cell_id_info_str);
		}
	}
...
int mi_dsi_display_get_fullscreen_aod_status(void *display, char *buf,
					     size_t size)
{
	struct dsi_display *dsi_display = (struct dsi_display *)display;
	...
	return snprintf(
		...
		dsi_display->panel->mi_cfg
			.feature_val[DISP_FEATURE_FULLSCREEN_AOD_STATUS]);
}
...
int mi_dsi_display_cmd_write_locked(struct dsi_display *display,
...
int mi_dsi_display_cmd_write(struct dsi_display *display,
			     struct dsi_panel_cmd_set *cmd_sets)
{
	int rc = 0;
...
ssize_t mi_dsi_display_get_debug_mafr(void *display, char *buf, size_t size)
{
	...
	return scnprintf(
		...
		dsi_display->panel->mi_cfg.mafr_enable_flag_ ? "enabled" :
							       "disabled",
		dsi_display->panel->mi_cfg.mafr_debug_switch ? "on" : "off",
		dsi_display->panel->mi_cfg.mafr_debug_feature_enable ?
			"enabled" :
			"disabled",
...
ssize_t mi_dsi_display_set_debug_mafr(void *display, char *input)
{
	...
	if (data_buf[0]) {
		...
		if (!data_buf[1]) {
			dsi_display->panel->mi_cfg.mafr_debug_feature_enable =
				false;
			ret = mi_dsi_panel_update_mafr_status(
				dsi_display->panel,
				&dsi_display->panel->mi_cfg
					 .last_surface_view_roi);
			if (ret)
	...
	if (config_updated) {
		...
		dsi_display->panel->mi_cfg.first_mafr_framerate =
			data_buf[1] ? data_buf[1] : 1;
		dsi_display->panel->mi_cfg.second_mafr_framerate =
			data_buf[2] ? data_buf[2] : 120;
		dsi_display->panel->mi_cfg.third_mafr_framerate =
			data_buf[3] ? data_buf[3] : 1;
		dsi_display->panel->mi_cfg.last_surface_view_roi.y =
			data_buf[4] ? data_buf[4] : 0;
		dsi_display->panel->mi_cfg.last_surface_view_roi.h =
			data_buf[5] ? data_buf[5] : 3200;
		dsi_display->panel->mi_cfg.last_surface_view_roi.x =
			data_buf[6] ? data_buf[6] : 0;
		dsi_display->panel->mi_cfg.last_surface_view_roi.w =
			data_buf[7] ? data_buf[7] : 1400;
...
module_param_string(panel_build_id, build_id_info_str, MAX_CMDLINE_PARAM_LEN,
		    0600);
MODULE_PARM_DESC(
	panel_build_id,
	"msm_drm.panel_build_id=<build id info> while <build id info> is 'panel build id info' ");

module_param_string(panel_cell_id, cell_id_info_str, MAX_CMDLINE_PARAM_LEN,
		    0600);
MODULE_PARM_DESC(
	panel_cell_id,
	"msm_drm.panel_cell_id=<cell id> while <cell id> is 'cell id info' ");

module_param_string(sec_cell_id, sec_cell_id_info_str, MAX_CMDLINE_PARAM_LEN,
		    0600);
MODULE_PARM_DESC(
	sec_cell_id,
	"msm_drm.sec_cell_id=<cell id> while <cell id> is 'cell id info' ");

module_param_string(debugpolicy, display_debug_policy,
		    MAX_DEBUG_POLICY_CMDLINE_LEN, 0600);
MODULE_PARM_DESC(
	debugpolicy,
	"msm_drm.debugpolicy=<debug policy> to indicate supporting ramdump or not ");
