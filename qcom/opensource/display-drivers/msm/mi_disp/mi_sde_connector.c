	...
	if (c_conn->connector_type == DRM_MODE_CONNECTOR_DSI) {
		...
		if (power_mode == SDE_MODE_DPMS_ON ||
			...
			msm_mode_object_event_notify(&c_conn->base.base,
						     ...
						     (u8 *)&c_conn->panel_dead);
			SDE_EVT32(SDE_EVTLOG_ERROR);
			DISP_ERROR("%s display esd irq check failed report"
				   " PANEL_DEAD conn_id: %d enc_id: %d\n",
				   display->display_type, c_conn->base.base.id,
				   c_conn->encoder->base.id);
		}
	}
	...
	if (c_conn->connector_type == DRM_MODE_CONNECTOR_DSI) {
		...
		if (display->panel->mi_cfg.esd_err_irq_gpio > 0) {
			rc = request_threaded_irq(
				display->panel->mi_cfg.esd_err_irq, NULL,
				mi_esd_err_irq_handle,
				display->panel->mi_cfg.esd_err_irq_flags,
				"esd_err_irq", c_conn);
			if (rc) {
				DISP_ERROR(
					"%s display register esd irq failed\n",
					display->display_type);
			} else {
				DISP_INFO(
					"%s display register esd irq success\n",
					display->display_type);
				disable_irq(display->panel->mi_cfg.esd_err_irq);
			}
#ifdef CONFIG_VIS_DISPLAY
			if (is_mi_dev_support_nova()) {
				vis_mi_esd_recover_handle_notify(
					mi_esd_err_irq_handle,
					display->panel->mi_cfg.esd_err_irq,
					c_conn);
			}
#endif
		}
...
int mi_sde_connector_debugfs_esd_sw_trigger(void *display)
{
	...
	c_conn->panel_dead = true;
	DISP_ERROR(
		"[esd-test]esd irq check failed report PANEL_DEAD conn_id: %d enc_id: %d\n",
	...
	DISP_INFO("[esd-test]%s display enabled (%d), power_mode (%s)\n",
		  dsi_display->display_type, dsi_display->enabled,
		  get_display_power_mode_name(power_mode));
	if (dsi_display->enabled) {
		if (power_mode == SDE_MODE_DPMS_ON ||
		    power_mode == SDE_MODE_DPMS_LP1) {
			sde_encoder_display_failure_notification(
				c_conn->encoder, false);
		}
	}

	event.type = DRM_EVENT_PANEL_DEAD;
	event.length = sizeof(bool);
	msm_mode_object_event_notify(&c_conn->base.base, c_conn->base.dev,
				     &event, (u8 *)&c_conn->panel_dead);
...
int mi_sde_connector_flat_fence(struct drm_connector *connector)
{
	...
	if (mi_get_disp_id(dsi_display->display_type) != MI_DISP_PRIMARY &&
	    mi_get_disp_id(dsi_display->display_type) != MI_DISP_SECONDARY)
		return -EINVAL;
		...
		if (flat_mode_val != mi_cfg->flat_cfg.cur_flat_state) {
			if (flat_mode_val == FEATURE_ON) {
				...
				dsi_panel_tx_cmd_set(
					dsi_display->panel,
					DSI_CMD_SET_MI_FLAT_MODE_ON, false);
				DISP_INFO(
					"DSI_CMD_SET_MI_FLAT_MODE_ON sync with te");
			} else {
				sde_encoder_wait_for_event(c_conn->encoder,
							   MSM_ENC_VBLANK);
				dsi_panel_tx_cmd_set(
					dsi_display->panel,
					DSI_CMD_SET_MI_FLAT_MODE_OFF, false);
				DISP_INFO(
					"DSI_CMD_SET_MI_FLAT_MODE_OFF sync with te");
			}
			mi_disp_feature_event_notify_by_type(
				mi_get_disp_id(dsi_display->display_type),
				MI_DISP_EVENT_FLAT_MODE, sizeof(flat_mode_val),
				flat_mode_val);
...
int mi_sde_connector_state_get_mi_mode_info(
	struct drm_connector_state *conn_state, struct mi_mode_info *mode_info)
{
						  ...
						  struct sde_kms_info *info)
{
	...
	list_for_each_entry(drm_mode, &conn->modes, head) {
		...
		if (rc) {
			DISP_ERROR(
				"conn%d failed to get mode info for mode %s\n",
					  ...
					  enum msm_mdp_conn_property prop_id)
{
	...
	msm_property_set_blob(&c_conn->property_info, blob,
			      SDE_KMS_INFO_DATA(info),
			      SDE_KMS_INFO_DATALEN(info), prop_id);
exit:
	vfree(info);
...
int mi_sde_connector_install_properties(struct sde_connector *c_conn)
{
	...
	if (c_conn->connector_type == DRM_MODE_CONNECTOR_DSI) {
		...
		msm_property_install_range(&c_conn->property_info,
					   "mi_mafr_info", 0x0, 0, U32_MAX, 0,
					   CONNECTOR_PROP_MI_MAFR_INFO);

		rc = mi_sde_connector_set_blob_data(
			&c_conn->base, CONNECTOR_PROP_MI_MODE_INFO);
		if (rc) {
			DISP_ERROR(
				"conn%d failed to setup connector info, rc = %d\n",
				c_conn->base.base.id, rc);
			return rc;
		}
	}
