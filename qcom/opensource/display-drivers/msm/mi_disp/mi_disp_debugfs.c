...
static int mi_disp_debugfs_mafr_debug_open(struct inode *inode,
					   struct file *file)
{
						...
						size_t count, loff_t *ppos)
{
	...
	input[count] = '\0';
	DISP_INFO(
		"mafr debug intput: {debug_enable fps_1 fps_2 fps_3 y h x w}: %s\n",
		input);
...
static int mi_disp_debugfs_mafr_debug_init(void *d_display, int disp_id)
{
	...
	if (is_support_disp_id(disp_id)) {
		disp_debugfs.mafr_debug[disp_id] =
			debugfs_create_file(mafr_debug_str[disp_id], S_IWUGO,
					    disp_core->debugfs_dir, dd_ptr,
					    &mafr_debug_debugfs_fops);
...
static int mi_disp_debugfs_backlight_log_init(void)
{
	...
	debugfs_create_u32(BACKLIGHT_LOG_DEBUGFS_NAME, S_IRUGO | S_IWUSR,
			   disp_core->debugfs_dir,
			   &disp_debugfs.backlight_log_mask);
...
static int mi_disp_debugfs_register_log_open(struct inode *inode,
					     struct file *file)
{
	struct disp_display *dd_ptr = inode->i_private;
						  ...
						  size_t count, loff_t *ppos)
{
	...
	register_buffer =
		kmalloc(sizeof(u8) * MAX_REGISTER_NUMBERS, GFP_KERNEL);
	if (!register_buffer) {
		kfree(input);
		return -ENOMEM;
	}
	...
	while (input) {
		...
		if (token) {
			...
			if (ret) {
				DISP_ERROR(
					"input buffer conversion failed or register value mismatch\n");
				goto exit;
			}
	...
	mutex_lock(&disp_debugfs.register_lock);
	memcpy(disp_debugfs.register_log_mask, register_buffer,
	       sizeof(u8) * MAX_REGISTER_NUMBERS);
	mutex_unlock(&disp_debugfs.register_lock);
...
static int mi_disp_debugfs_register_log_init(void *d_display, int disp_id)
{
	...
	if (is_support_disp_id(disp_id)) {
		disp_debugfs.register_log =
			debugfs_create_file(REGISTER_LOG_DEBUGFS_NAME, S_IWUGO,
					    disp_core->debugfs_dir, dd_ptr,
					    &register_log_debugfs_fops);
		if (!disp_debugfs.register_log) {
...
static int mi_disp_debugfs_esd_sw_init(void *d_display, int disp_id)
{
	...
	if (is_support_disp_id(disp_id)) {
		disp_debugfs.esd_sw[disp_id] = debugfs_create_file(
			esd_sw_str[disp_id], S_IWUGO, disp_core->debugfs_dir,
...
static int mi_disp_debugfs_param_debug_open(struct inode *inode,
					    struct file *file)
{
	struct disp_display *dd_ptr = inode->i_private;
...
static int mi_disp_debugfs_param_debug_init(void *d_display, int disp_id)
{
	...
	if (is_support_disp_id(disp_id)) {
		disp_debugfs.param_debug[disp_id] =
			debugfs_create_file(param_debug_str[disp_id], S_IWUGO,
					    disp_core->debugfs_dir, dd_ptr,
					    &param_debug_debugfs_fops);
