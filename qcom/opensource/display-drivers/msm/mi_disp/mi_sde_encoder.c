...
void mi_sde_encoder_save_vsync_info(struct sde_encoder_phys *phys_enc)
{
	...
	diff_us =
		(u64)ktime_us_delta(current_ktime, calc_vsync->last_timestamp);
	...
	calc_vsync->vsyc_info[calc_vsync->vsync_count].config_fps =
		info->frame_rate;
	calc_vsync->vsyc_info[calc_vsync->vsync_count].timestamp =
		current_ktime;
	calc_vsync->vsyc_info[calc_vsync->vsync_count].real_vsync_period_ns =
		ktime_to_ns(
			ktime_sub(current_ktime, calc_vsync->last_timestamp));
					  ...
					  char *buf, size_t size)
{
	...
	index = (calc_vsync->vsync_count == 0) ? (MAX_VSYNC_COUNT - 1) :
						 (calc_vsync->vsync_count - 1);
	...
	for (i = 0; i < MAX_VSYNC_COUNT; i++) {
		if (fps == calc_vsync->vsyc_info[index].config_fps) {
			diff_us = (u64)ktime_us_delta(
				current_time,
				calc_vsync->vsyc_info[index].timestamp);
			if (diff_us <= USEC_PER_SEC) {
				valid_total_vsync_period_ns +=
					calc_vsync->vsyc_info[index]
						.real_vsync_period_ns;
				valid_count++;
			}
			total_vsync_period_ns += calc_vsync->vsyc_info[index]
							 .real_vsync_period_ns;
			count++;
		}
	...
	calc_vsync->measured_fps_x1000 =
		(u32)((NSEC_PER_SEC * 1000) /
		      calc_vsync->measured_vsync_period_ns);

	return scnprintf(buf, size,
			 "fps: %d.%d vsync_period_ns:%lld"
