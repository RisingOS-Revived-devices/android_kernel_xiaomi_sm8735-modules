...
static void mi_disp_ioctl_set_count_info_work_handler(struct kthread_work *work)
{
	...
	struct disp_count_info *count_info =
		(struct disp_count_info *)cur_work->data;
						   ...
						   void *data, u32 size)
{
	...
	kthread_init_work(&cur_work->work,
			  mi_disp_ioctl_set_count_info_work_handler);
	cur_work->dd_ptr = dd_ptr;
	cur_work->wq = &dd_ptr->pending_wq;
	cur_work->data = (u8 *)cur_work + sizeof(struct disp_work);
	memcpy(cur_work->data, data, size);
...
static int mi_disp_ioctl_set_count_info(struct disp_feature_client *client,
					void *data)
{
	struct disp_feature *df = client->df;
	...
	if (is_support_disp_id(disp_id)) {
		...
		if (dd_ptr->intf_type == MI_INTF_DSI) {
			...
			DISP_DEBUG("%s display set feature: %s, value: %d\n",
				   get_disp_id_name(disp_id),
				   get_disp_count_info_type_name(
					   count_info.count_info_type),
				   count_info.count_info_val);

			if (is_support_disp_count_info_type(
				    count_info.count_info_type)) {
				if (req->tx_len) {
					count_info.tx_len = req->tx_len;
					count_info.tx_ptr = kzalloc(
						count_info.tx_len, GFP_KERNEL);
				...
				if (req->rx_len) {
					...
					count_info.rx_ptr = kzalloc(
						count_info.rx_len, GFP_KERNEL);
				...
				if (req->base.flag == MI_DISP_FLAG_NONBLOCK) {
					ret = mi_disp_ioctl_set_count_info_queue_work(
						dd_ptr, &count_info,
						sizeof(count_info));
				} else {
					ret = mi_dsi_display_set_count_info(
						dd_ptr->display, &count_info);
				}
				...
				DISP_ERROR(
					"unsupported disp count info type\n");
				ret = -EINVAL;
				goto exit;
			}
		} else {
...
static int mi_disp_ioctl_set_fp_unlock_state(struct disp_feature_client *client,
					     void *data)
{
	struct disp_feature *df = client->df;
	...
	if (is_support_disp_id(disp_id)) {
		dd_ptr = &df->d_display[disp_id];
		if (dd_ptr->intf_type == MI_INTF_DSI) {
			ret = mi_dsi_display_set_fp_unlock_state(
				dd_ptr->display, fp_unlock_value);
		} else {
			DISP_INFO("Unsupported display(%s intf)\n",
				  get_disp_intf_type_name(dd_ptr->intf_type));
			ret = -EINVAL;
...
static const struct disp_ioctl_desc disp_ioctls[] = {
	...
	DISP_IOCTL_DEF(MI_DISP_IOCTL_SET_COUNT_INFO,
		       mi_disp_ioctl_set_count_info),
	DISP_IOCTL_DEF(MI_DISP_IOCTL_SET_FP_UNLOCK_STATE,
		       mi_disp_ioctl_set_fp_unlock_state),
};
