...
#define INC_HEAD(head, max_entries, ret) \
	div_u64_rem((++head), max_entries, (ret))
...
int cam_debug_record_key_message(int subdev_id, void *ctrl, void *info,
				 uint64_t request_id, int32_t idx, int opcode)
{
	int rc = 0;
	long microsec = 0;
	...
	CAM_DBG(CAM_UTIL, "device: %s operation: %s,seq_id: %llu",
		get_subdev_name(subdev_id), get_opcode_name(opcode),
		request_id);
	if (cam_key_buffer == NULL || cam_io_record_buffer == NULL ||
	...
	if (subdev_id == CAM_ISP) {
		...
		if (opcode == EVENT_ISP_SOF) {
			...
			s_ctrl =
				(struct cam_sensor_ctrl_t *)cam_get_device_priv(
					dev->dev_hdl);
			soc_info = &s_ctrl->soc_info;
			INC_HEAD(
				cam_key_buffer->event_record_head[EVENT_ISP_SOF],
				MAX_RECORD_REQ, &index);
			cam_key_buffer->event_id[EVENT_ISP_SOF][index].opcode =
				EVENT_ISP_SOF;
			if (soc_info) {
				strlcpy(cam_key_buffer
						->event_id[EVENT_ISP_SOF][index]
						.u.frame_info.dev_name,
					soc_info->dev_name,
					CAM_SENSOR_NAME_MAX_LENGTH);
			}
			cam_key_buffer->event_id[EVENT_ISP_SOF][index]
				.u.frame_info.frame_id = ctx_isp->frame_id;
			CAM_CONVERT_TIMESTAMP_FORMAT(
				ts,
				cam_key_buffer->event_id[EVENT_ISP_SOF][index]
					.ts_h,
				cam_key_buffer->event_id[EVENT_ISP_SOF][index]
				...
				cam_key_buffer->event_id[EVENT_ISP_SOF][index]
					.ts_ms);
			cam_key_buffer->event_id[EVENT_ISP_SOF][index].req_id =
				req_id;
		} else if (opcode == EVENT_ISP_EOF) {
			INC_HEAD(
				cam_key_buffer->event_record_head[EVENT_ISP_EOF],
				MAX_RECORD_REQ, &index);
			cam_key_buffer->event_id[EVENT_ISP_EOF][index].opcode =
				EVENT_ISP_EOF;
			CAM_CONVERT_TIMESTAMP_FORMAT(
				ts,
				cam_key_buffer->event_id[EVENT_ISP_EOF][index]
					.ts_h,
				cam_key_buffer->event_id[EVENT_ISP_EOF][index]
					.ts_m,
				cam_key_buffer->event_id[EVENT_ISP_EOF][index]
			...
			INC_HEAD(cam_key_buffer
					 ->event_record_head[EVENT_ISP_EPOCH],
				 MAX_RECORD_REQ, &index);
			cam_key_buffer->event_id[EVENT_ISP_EPOCH][index].opcode =
				EVENT_ISP_EPOCH;
			CAM_CONVERT_TIMESTAMP_FORMAT(
				ts,
				cam_key_buffer->event_id[EVENT_ISP_EPOCH][index]
					.ts_h,
				cam_key_buffer->event_id[EVENT_ISP_EPOCH][index]
					.ts_m,
				cam_key_buffer->event_id[EVENT_ISP_EPOCH][index]
			...
			soc_info = &s_ctrl->soc_info;
			INC_HEAD(
				cam_key_buffer->event_record_head[EVENT_ADD_REQ],
				MAX_RECORD_REQ, &index);
			cam_key_buffer->event_id[EVENT_ADD_REQ][index].opcode =
				EVENT_ADD_REQ;
			CAM_CONVERT_TIMESTAMP_FORMAT(
				ts,
				cam_key_buffer->event_id[EVENT_ADD_REQ][index]
					.ts_h,
				cam_key_buffer->event_id[EVENT_ADD_REQ][index]
					.ts_m,
				cam_key_buffer->event_id[EVENT_ADD_REQ][index]
			...
			cam_key_buffer->event_id[EVENT_ADD_REQ][index].req_id =
				add_request->req_id;
			cam_key_buffer->event_id[EVENT_ADD_REQ][index]
				.u.add_req.module_id = soc_info->index;
			cam_key_buffer->event_id[EVENT_ADD_REQ][index]
				.u.add_req.subdev_id = subdev_id;
			cam_key_buffer->event_id[EVENT_ADD_REQ][index]
				.u.add_req.dev_hdl = add_request->dev_hdl;
			cam_key_buffer->event_id[EVENT_ADD_REQ][index]
				.u.add_req.link_hdl = add_request->link_hdl;
			...
			soc_info = &s_ctrl->soc_info;
			INC_HEAD(cam_key_buffer
					 ->event_record_head[EVENT_APPLY_REQ],
				 MAX_RECORD_REQ, &index);
			cam_key_buffer->event_id[EVENT_APPLY_REQ][index].opcode =
				EVENT_APPLY_REQ;
			CAM_CONVERT_TIMESTAMP_FORMAT(
				ts,
				cam_key_buffer->event_id[EVENT_APPLY_REQ][index]
					.ts_h,
				cam_key_buffer->event_id[EVENT_APPLY_REQ][index]
					.ts_m,
				cam_key_buffer->event_id[EVENT_APPLY_REQ][index]
			...
			cam_key_buffer->event_id[EVENT_APPLY_REQ][index].req_id =
				request_id;
			cam_key_buffer->event_id[EVENT_APPLY_REQ][index]
				.u.app_req.link_hdl = apply->link_hdl;
			if (soc_info)
				cam_key_buffer->event_id[EVENT_APPLY_REQ][index]
					.u.app_req.module_id = soc_info->index;
			cam_key_buffer->event_id[EVENT_APPLY_REQ][index]
				.u.app_req.subdev_id = subdev_id;
		}
	} else if (opcode == EVENT_POWER_UP) {
		INC_HEAD(cam_power_record_buffer->event_record_head[0],
			 MAX_RECORD_POWER, &index);
		CAM_CONVERT_TIMESTAMP_FORMAT(
			ts, cam_power_record_buffer->power_seq[0][index].ts_h,
		...
		cam_power_record_buffer->power_seq[0][index].module_id =
			soc_info->index;
		cam_power_record_buffer->power_seq[0][index].subdev_id =
			get_subdev_id(soc_info);
		switch (power_setting->seq_type) {
		case SENSOR_MCLK:
			if (power_setting->config_val)
				cam_power_record_buffer->power_seq[0][index]
					.clk_rate = power_setting->config_val;
			cam_power_record_buffer->power_seq[0][index]
				.power_seq_msg = power_setting->seq_type;
			cam_power_record_buffer->power_seq[0][index].gpio_val =
				0;
			min_volt = soc_info->rgltr_min_volt[idx];
			max_volt = soc_info->rgltr_max_volt[idx];
			cam_power_record_buffer->power_seq[0][index].voltage[0] =
				min_volt;
			cam_power_record_buffer->power_seq[0][index].voltage[1] =
				max_volt;
			break;
		case SENSOR_RESET:
		case SENSOR_STANDBY:
		...
		case SENSOR_CUSTOM_GPIO2:
			cam_power_record_buffer->power_seq[0][index].clk_rate =
				0;
			cam_power_record_buffer->power_seq[0][index]
				.power_seq_msg = power_setting->seq_type;
			cam_power_record_buffer->power_seq[0][index].gpio_val =
		...
		INC_HEAD(cam_power_record_buffer->event_record_head[1],
			 MAX_RECORD_POWER, &index);
		CAM_CONVERT_TIMESTAMP_FORMAT(
			ts, cam_power_record_buffer->power_seq[1][index].ts_h,
		...
		cam_power_record_buffer->power_seq[1][index].module_id =
			soc_info->index;
		cam_power_record_buffer->power_seq[1][index].subdev_id =
			get_subdev_id(soc_info);
		switch (power_setting->seq_type) {
		case SENSOR_MCLK:
			if (power_setting->config_val)
				cam_power_record_buffer->power_seq[1][index]
					.clk_rate = power_setting->config_val;
			cam_power_record_buffer->power_seq[1][index]
				.power_seq_msg = power_setting->seq_type;
			cam_power_record_buffer->power_seq[1][index].gpio_val =
				0;
			min_volt = soc_info->rgltr_min_volt[idx];
			max_volt = soc_info->rgltr_max_volt[idx];
			cam_power_record_buffer->power_seq[1][index].voltage[0] =
				min_volt;
			cam_power_record_buffer->power_seq[1][index].voltage[1] =
				max_volt;
			break;
		case SENSOR_RESET:
		case SENSOR_STANDBY:
		case SENSOR_CUSTOM_GPIO1:
		case SENSOR_CUSTOM_GPIO2:
			cam_power_record_buffer->power_seq[1][index].clk_rate =
				0;
			cam_power_record_buffer->power_seq[1][index]
				.power_seq_msg = power_setting->seq_type;
			cam_power_record_buffer->power_seq[1][index].gpio_val =
		...
		if (CAM_SENSOR == subdev_id) {
			INC_HEAD(cam_io_record_buffer->event_record_head[0],
				 APPLY_SETTING_REQ, &index);
			CAM_CONVERT_TIMESTAMP_FORMAT(
				ts,
				cam_io_record_buffer->sensor_setting_data[index]
					.ts_h,
				cam_io_record_buffer->sensor_setting_data[index]
					.ts_m,
				cam_io_record_buffer->sensor_setting_data[index]
				...
				cam_io_record_buffer->sensor_setting_data[index]
					.ts_ms);
			cam_io_record_buffer->sensor_setting_data[index].req_id =
				request_id;
			...
			cam_io_record_buffer->sensor_setting_data[index]
				.module_id = soc_info->index;
			cam_io_record_buffer->sensor_setting_data[index]
				.subdev_id = subdev_id;
			cam_io_record_buffer->sensor_setting_data[index].opcode =
				i2c_set->op_code;
			cam_io_record_buffer->sensor_setting_data[index]
				.apply_record_size = idx;
			cam_io_record_buffer->sensor_setting_data[index]
				.regAddrType = i2c_set->i2c_settings.addr_type;
			cam_io_record_buffer->sensor_setting_data[index]
				.regDataType = i2c_set->i2c_settings.data_type;
			for (IO_index = 0; IO_index < idx &&
					   IO_index < MAX_RECORD_SENSOR_SETTING;
			     ++IO_index) {
				cam_io_record_buffer->sensor_setting_data[index]
					.ad[IO_index]
					.address =
					i2c_set->i2c_settings
						.reg_setting[IO_index]
						.reg_addr;
				cam_io_record_buffer->sensor_setting_data[index]
					.ad[IO_index]
					.data = i2c_set->i2c_settings
							.reg_setting[IO_index]
							.reg_data;
			}
		} else if (CAM_OIS == subdev_id) {
			INC_HEAD(cam_io_record_buffer->event_record_head[1],
				 APPLY_SETTING_REQ, &index);
			CAM_CONVERT_TIMESTAMP_FORMAT(
				ts,
				cam_io_record_buffer->ois_setting[index].ts_h,
			...
			cam_io_record_buffer->ois_setting[index].req_id =
				request_id;

			soc_info = (struct cam_hw_soc_info *)ctrl;
			i2c_set = (struct i2c_settings_list *)info;

			cam_io_record_buffer->ois_setting[index].module_id =
				soc_info->index;
			cam_io_record_buffer->ois_setting[index].subdev_id =
				subdev_id;
			cam_io_record_buffer->ois_setting[index].opcode =
				i2c_set->op_code;
			cam_io_record_buffer->ois_setting[index]
				.apply_record_size = idx;

			for (IO_index = 0; IO_index < idx &&
					   IO_index < MAX_RECORD_OTHER_SETTING;
			     ++IO_index) {
				cam_io_record_buffer->ois_setting[index]
					.ad[IO_index]
					.address =
		...
		if (CAM_SENSOR == subdev_id) {
			div_u64_rem(cam_io_record_buffer->event_record_head[0],
				    APPLY_SETTING_REQ, &index);
			CAM_CONVERT_TIMESTAMP_FORMAT(
				ts,
				cam_io_record_buffer->sensor_setting_data[index]
			...
			div_u64_rem(cam_io_record_buffer->event_record_head[1],
				    APPLY_SETTING_REQ, &index);
			CAM_CONVERT_TIMESTAMP_FORMAT(
				ts,
				cam_io_record_buffer->ois_setting[index]
		...
		INC_HEAD(cam_key_buffer->event_record_head[EVENT_ADD_REQ],
			 MAX_RECORD_REQ, &index);
		cam_key_buffer->event_id[EVENT_ADD_REQ][index].opcode =
			EVENT_ADD_REQ;
		CAM_CONVERT_TIMESTAMP_FORMAT(
			ts, cam_key_buffer->event_id[EVENT_ADD_REQ][index].ts_h,
		...
		INC_HEAD(cam_key_buffer->event_record_head[EVENT_APPLY_REQ],
			 MAX_RECORD_REQ, &index);
		cam_key_buffer->event_id[EVENT_APPLY_REQ][index].opcode =
			EVENT_APPLY_REQ;
		CAM_CONVERT_TIMESTAMP_FORMAT(
			ts,
			cam_key_buffer->event_id[EVENT_APPLY_REQ][index].ts_h,
	...
	CAM_DBG(CAM_UTIL, "%s end %s, occupy time is: %ld ms",
		get_subdev_name(subdev_id), get_opcode_name(opcode),
		microsec / 1000);
	return rc;
}
...
void cam_init_message_buffer(void)
{
	if (cam_key_buffer != NULL || cam_io_record_buffer != NULL ||
		...
		if (cam_io_record_buffer != NULL)
			CAM_DBG(CAM_UTIL,
				"cam_io_record_buffer has been created");
		return;
	}

	CAM_ERR(CAM_UTIL, "io_record_size:%d",
		sizeof(struct cam_io_data_record));
	cam_key_buffer =
		vmalloc(sizeof(struct cam_debug_record_key_message_buffer));
	...
	CAM_ERR(CAM_UTIL, "cam_debug_record_key_message_buffer:%d",
		sizeof(struct cam_debug_record_key_message_buffer));

	if (!cam_key_buffer || !cam_io_record_buffer ||
		...
		if (!cam_io_record_buffer) {
			CAM_ERR(CAM_UTIL,
				"vmalloc cam_io_record_buffer failure");
		}
		if (!cam_power_record_buffer) {
			CAM_ERR(CAM_UTIL,
				"vmalloc cam_power_record_buffer failure");
		}
...
void dump_sensor_setting(void)
{
	...
	for (int i = 0; i < record_count; ++i) {
		CAM_ERR(CAM_UTIL,
			"Sensor_Setting_Apply_start:TS:%llu:%llu:%llu:%llu,module_id:%u,subdev_id:%hd,opcode:%hu,apply_record_size:%d,req_id:%llu",
			...
			cam_io_record_buffer->sensor_setting_data[i]
				.apply_record_size,
			cam_io_record_buffer->sensor_setting_data[i].req_id);
		for (int k = 0; k < cam_io_record_buffer->sensor_setting_data[i]
						.apply_record_size &&
				k < MAX_RECORD_SENSOR_SETTING;
		     ++k) {
		...
		CAM_ERR(CAM_UTIL,
			"Sensor_Setting_Apply_End:TS:%llu:%llu:%llu:%llu,req_id:%llu,subdev_id:%hd",
