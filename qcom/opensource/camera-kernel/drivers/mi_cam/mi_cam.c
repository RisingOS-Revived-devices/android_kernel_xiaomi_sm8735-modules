...
static u64 mem_stall_pct_max =
	DEFAULT_MEM_STALL_PCT_MAX; //cutoff stall ratio by percentage
...
static bool micam_read_amu_counter_delta(struct micam_amu_data *arch_data)
{
	...
	if (unlikely(!(mem_stall && amu_cyc))) {
		...
		if (unlikely(mem_stall <
			     arch_data->mem_stall)) { //MEM_STALL counter wrap
			arch_data->mem_stall_delta =
				U64_MAX - arch_data->mem_stall + mem_stall;
		} else {
			arch_data->mem_stall_delta =
				mem_stall - arch_data->mem_stall;
		}

		if (unlikely(amu_cyc <
			     arch_data->amu_cyc)) { //CPU cycles counter wrap
			arch_data->amu_cyc_delta =
				U64_MAX - arch_data->amu_cyc + amu_cyc;
		} else {
		...
		trace_camsched_read_amu_counter(arch_data->cpu, mem_stall,
						amu_cyc,
						arch_data->mem_stall_delta,
						arch_data->amu_cyc_delta,
						arch_data->last_update);
		return true;
	}
}
...
u64 micam_calibrate_cpu_cycles(u64 wallclock, int cpu, u64 raw_cycles_delta)
{
	...
	struct micam_amu_data *arch_data =
		&per_cpu(micam_amu_data, raw_smp_processor_id());
	...
	mem_stall_pct =
		mult_frac(mem_stall_delta, mem_stall_delta_mult, amu_cyc_delta);
	...
	if (likely(amu_cyc_delta > mem_stall_delta)) {
		caled_cycles_delta =
			raw_cycles_delta -
			mult_frac(calc_data1, raw_cycles_delta, 100);
	...
	trace_camsched_calibrate_cpu_cycles(stage, cpu, caled_cycles_delta,
					    raw_cycles_delta, amu_cyc_delta,
					    mem_stall_delta, calc_data1);
...
ssize_t micam_read(struct file *file, char __user *buff, size_t length,
		   loff_t *ppos)
{
...
ssize_t micam_write(struct file *file, const char __user *buf, size_t length,
		    loff_t *ppos)
{
...
static long micam_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	case MI_CAM_SCHED_ENABLE:
		...
		write_unlock(&micam_dev->f_lock);
		mod_timer(&micam_dev->crm_wdog,
			  jiffies + msecs_to_jiffies(5000)); //5s
...
static int __init mi_camera_init(void)
{
	...
	device = device_create(micam_class, NULL, micam_dev->dev, NULL,
			       MI_CAM_SCHED);
	if (IS_ERR(device)) {
	...
	if (device != NULL) {
		rc = sysfs_create_group(&device->kobj,
					&cam_cpubg_pressure_attr_group);
		if (rc) {
...
static void __exit mi_cam_exit(void)
{
	...
	sysfs_remove_group(&micam_dev->micam_device->kobj,
			   &cam_cpubg_pressure_attr_group);
