	...
	list_for_each_entry(i2c_list, &(i2c_set->list_head), list) {
		...
		case CAM_SENSOR_I2C_WRITE_SEQ: {
			for (j = 0; j < i2c_list->i2c_settings.size; j++) {
				CAM_DBG(CAM_OIS,
					"[sem1217s] ois name %s, request id %d, j=[%d], reg_addr 0x%x, 0x%x",
					o_ctrl->ois_name, i2c_set->request_id,
					j,
					i2c_list->i2c_settings.reg_setting[j]
						.reg_addr,
					i2c_list->i2c_settings.reg_setting[j]
						.reg_data);
				trace_cam_i2c_write_log_event(
					"[sem1217s]", o_ctrl->ois_name,
					i2c_set->request_id, j, "WRITE",
					i2c_list->i2c_settings.reg_setting[j]
						.reg_addr,
					i2c_list->i2c_settings.reg_setting[j]
						.reg_data);
...
static int32_t i2c_read_data(struct cam_ois_ctrl_t *o_ctrl, uint32_t addr,
			     uint32_t length, uint8_t *data)
{
	int32_t rc = 0;
	int32_t i = 0;

	rc = camera_io_dev_read_seq(&o_ctrl->io_master_info, addr, data,
				    CAMERA_SENSOR_I2C_TYPE_WORD,
				    CAMERA_SENSOR_I2C_TYPE_BYTE, length);

	for (i = 0; i < length; i++) {
		CAM_DBG(CAM_OIS, "[SEM1217S] read addr 0x%04x[%d] = 0x%02x",
			addr, i, data[i]);
	}
...
static int32_t i2c_write_data(struct cam_ois_ctrl_t *o_ctrl, uint32_t addr,
			      uint32_t length, uint8_t *data, uint32_t delay)
{
	...
	if (rc < 0) {
		CAM_ERR(CAM_OIS,
			"[SEM1217S] OIS i2c_write_data write failed, rc: %d",
			rc);
	}

	for (i = 0; i < length && i < 256; i += 4) {
		CAM_DBG(CAM_OIS,
			"[SEM1217S] Write addr 0x%04x = 0x%02x 0x%02x 0x%02x 0x%02x",
			w_data[i].reg_addr, data[i], data[i + 1], data[i + 2],
			data[i + 3]);
	}
...
static int32_t load_fw_buff(struct cam_ois_ctrl_t *o_ctrl, char *firmware_name,
...
int sem1217s_ois_fw_download(struct cam_ois_ctrl_t *o_ctrl)
{
	...
	if (rc) {
		CAM_ERR(CAM_OIS, "[SEM1217S] Failed to load firmware: %s",
			fw_name_prog);
		goto memory_free;
	}
	...
	if (rc) {
		CAM_ERR(CAM_OIS, "[SEM1217S] Failed to read REG_APP_VER:0x%x",
			REG_APP_VER);
		rc = -EIO;
		goto memory_free;
	}
	...
	CAM_INFO(
		CAM_OIS,
		"[SEM1217S] Current firmware version = %d, new firmware version = %d, noUpdateForOisWithFlash %d",
		current_fw_ver, new_fw_ver,
		o_ctrl->opcode.noUpdateForOisWithFlash);

	if (((((current_fw_ver < new_fw_ver) && (0 == semco_ois_fw_update_op)) ||
	      (current_fw_ver != new_fw_ver)) &&
	     (FALSE == o_ctrl->opcode.noUpdateForOisWithFlash)) ||
	    (FIRMWARE_UPDATE_EVERY_TIMES == semco_ois_fw_update_op)) {
		/* If there is firmware that needs to be updated, turn off OIS and AF */
		if (0 != current_fw_ver) {
			rc = i2c_read_data(o_ctrl, REG_OIS_STS, 1,
			...
			if (rxdata[0] != STATE_READY) {
				...
				rc = i2c_write_data(o_ctrl, REG_OIS_CTRL, 1,
						    txdata, 0);
				if (rc) {
					CAM_ERR(CAM_OIS,
			...
			rc = i2c_read_data(o_ctrl, REG_AF_STS, 1,
			...
			if (rxdata[0] != STATE_READY) {
				...
				rc = i2c_write_data(o_ctrl, REG_AF_CTRL, 1,
						    txdata, 0);
				if (rc) {
					CAM_ERR(CAM_OIS,
		...
		if (STATE_FW_UPDATE != rxdata[0]) {
			CAM_INFO(
				CAM_OIS,
				"[SEM1217S] OIS firmware upgrade status check failed");
			rc = -EINVAL;
			goto memory_free;
		}
		...
		if (rc) {
			CAM_ERR(CAM_OIS,
				"[SEM1217S] Failed to read REG_APP_VER:0x%x",
				REG_APP_VER);
			rc = -EIO;
			goto memory_free;
		}
		...
		if (0x00 != current_fw_ver_temp) {
			CAM_ERR(CAM_OIS,
				"[SEM1217S] OIS firmware version check failed");
			goto memory_free;
		}

		for (i = 0; i < (APP_FW_SIZE / tx_buff_size); i++) {
			CAM_INFO(CAM_OIS,
				 "[SEM1217S] Write REG_DATA_BUF i = %d", i);
			memcpy(&chk_buffer[tx_buff_size * i], &fw_data[idx],
			       tx_buff_size);

			for (chk_index = 0; chk_index < tx_buff_size;
			     chk_index += 2) {
				check_sum += ((chk_buffer[chk_index + 1 +
							  (tx_buff_size * i)]
					       << 8) |
					      chk_buffer[chk_index +
							 (tx_buff_size * i)]);
			}

			memcpy(txdata, &fw_data[idx], tx_buff_size);

			rc = i2c_write_data(o_ctrl, REG_DATA_BUF, tx_buff_size,
					    txdata, 0);
			if (rc) {
				CAM_ERR(CAM_OIS,
		...
		*(uint16_t *)txdata = check_sum;
		...
		CAM_INFO(CAM_OIS, "[SEM1217S] REG_FWUP_CHKSUM = 0x%x, 0x%x",
			 rxdata[0], rxdata[1]);

		rc = i2c_read_data(o_ctrl, REG_FWUP_ERR, 1, rxdata);
		if (rc) {
			CAM_ERR(CAM_OIS,
				"[SEM1217S] Failed to read REG_FWUP_ERR:0x%x",
				REG_FWUP_ERR);
			rc = -EIO;
			goto memory_free;
		}
		...
		if (rxdata[0] != NO_ERROR) {
			CAM_ERR(CAM_OIS,
				"[SEM1217S] Failed to update firmware");
			rc = -EINVAL;
			goto memory_free;
		}
		...
		if (updated_ver != new_fw_ver) {
			CAM_ERR(CAM_OIS,
				"[SEM1217S] updated_ver != new_fw_ver");
			rc = -EINVAL;
			goto memory_free;
		}
	...
	if (o_ctrl->is_ois_calib) {
		...
		if ((rc == -EAGAIN) &&
			...
			CAM_WARN(
				CAM_OIS,
				"[sem1217s] CCI HW is restting: Reapplying calib settings");
			usleep_range(1000, 1010);
			rc = sem1217s_ois_apply_settings(
				o_ctrl, &o_ctrl->i2c_calib_data);
		}
		if (rc) {
			CAM_ERR(CAM_OIS,
				"[sem1217s] Cannot apply calib data %d flag %d",
				rc, o_ctrl->opcode.customized_ois_flag);
			//	goto pwr_dwn;
		} else {
			CAM_DBG(CAM_OIS,
				"[sem1217s] apply calib data settings success");
		}
	}
	...
	if (o_ctrl->i2c_init_data.is_settings_valid == 1) {
		...
		if (rc == -EAGAIN) {
			CAM_WARN(
				CAM_OIS,
				"[sem1217s] CCI HW is restting: Reapplying i2c_init_data settings");
			usleep_range(1000, 1010);
			rc = sem1217s_ois_apply_settings(
				o_ctrl, &o_ctrl->i2c_init_data);
		}
		if (rc) {
			CAM_ERR(CAM_OIS,
				"Cannot apply i2c_init_data data %d, flag %d",
				rc, o_ctrl->opcode.customized_ois_flag);
			//	goto pwr_dwn;
		} else {
			CAM_DBG(CAM_OIS,
				"[sem1217s] OIS program Flash settings success");
		}
	}
