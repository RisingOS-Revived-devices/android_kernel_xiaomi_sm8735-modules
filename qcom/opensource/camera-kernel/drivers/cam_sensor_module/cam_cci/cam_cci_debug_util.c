...
DEFINE_DEBUGFS_ATTRIBUTE(cam_cci_debug_val_readonly, cam_cci_debug_get_val,
			 NULL, "%llu\n");
...
DEFINE_DEBUGFS_ATTRIBUTE(cam_cci_debug_val, cam_cci_debug_get_val,
			 cam_cci_debug_set_val, "%llu\n");
...
DEFINE_DEBUGFS_ATTRIBUTE(cam_cci_debug_addr_type, cam_cci_debug_get_val,
			 cam_cci_debug_set_addr_type, "%llu\n");
...
DEFINE_DEBUGFS_ATTRIBUTE(cam_cci_debug_data_type, cam_cci_debug_get_val,
			 cam_cci_debug_set_data_type, "%llu\n");
...
{
	if (type == CAMERA_SENSOR_I2C_TYPE_WORD && *data > MAX_DEV_TYPE_WORD) {
		CAM_WARN(
			MI_DEBUG,
			"Start address exceeds upper limit then modified to: %d",
			MAX_DEV_TYPE_BYTE);
					 ...
					 size_t count, loff_t *ppos)
{
	...
	if (kstrtoul(start, 16, &reg_end)) {
		CAM_INFO(MI_DEBUG, "Set to single register read: %d",
			 reg_start);
	...
	if (reg_end < reg_start) {
		CAM_WARN(
			MI_DEBUG,
			"End address is lower than start address then modified to: %d",
			reg_start);
		reg_end = reg_start;
	}
	...
	CAM_DBG(MI_DEBUG, "Register read range set to: %d - %d", reg_start,
		reg_end);
					...
					enum camera_sensor_i2c_type data_type)
{
	...
	rc = camera_io_dev_read_seq(io_master_info, start, data, addr_type,
				    data_type, end - start + 1);
	if (rc < 0) {
					  ...
					  loff_t *ppos)
{
	...
	CAM_DBG(MI_DEBUG, "Monitor registers from 0x%x to 0x%x", start, end);

	if (end - start + 1 > MAX_DEV_MONITOR_COUNT) {
		end = start + MAX_DEV_MONITOR_COUNT - 1;
		CAM_WARN(MI_DEBUG,
			 "Max monitor regs count is %d, "
			 "reset regs to 0x%x - 0x%x",
			 MAX_DEV_MONITOR_COUNT, start, end);
	}
	...
	rc = __cam_cci_debug_cci_read_seq(cci_dbg->io_master_info, data, start,
					  end, cci_dbg->reg_addr_type,
	...
	hrs = hrs + TIME_SHIFT_HOURS % TIME_MAX_HOURS;
	snprintf(buf + buf_pos, count - buf_pos,
		 "--- %.*llu:%.*llu:%.*llu.%.*llu\n", PRINT_SINGLE_TIME, hrs,
		 PRINT_SINGLE_TIME, min, PRINT_SINGLE_TIME, sec,
		 PRINT_SINGLE_TIME, ms);
	buf_pos += PRINT_TIMESTAMP;

	for (i = start; i <= end; i++) {
		if ((buf_pos + cci_dbg->reg_addr_type * 2 +
		     PRINT_FIXED_LENGTH_4 + cci_dbg->reg_data_type * 2 + 1) >
		    count)
			break;

		snprintf(buf + buf_pos, count - buf_pos,
			 "0x%.*x: ", cci_dbg->reg_addr_type * 2, i);
		/* (+ 4) for '0x' and ': ' */
		buf_pos += cci_dbg->reg_addr_type * 2 + PRINT_FIXED_LENGTH_4;

		snprintf(buf + buf_pos, count - buf_pos, "%.*x",
			 cci_dbg->reg_data_type * 2,
			 data[i - cci_dbg->min_reg_addr]);
		buf_pos += cci_dbg->reg_data_type * 2;

		buf[buf_pos++] = '\n';

		CAM_DBG(MI_DEBUG, "CCI debug read register 0x%x data 0x%x", i,
			data[i - cci_dbg->min_reg_addr]);
	}
...
static ssize_t cam_cci_debug_regs_read(struct file *file, char __user *user_buf,
				       size_t count, loff_t *ppos)
{
	struct cam_cci_debug *cci_dbg =
		(struct cam_cci_debug *)file->private_data;
	...
	CAM_DBG(MI_DEBUG,
		"Read registers from 0x%x to 0x%x, "
	...
	if (*end - start + 1 > I2C_REG_DATA_MAX) {
		*end = start + I2C_REG_DATA_MAX - 1;
		CAM_WARN(MI_DEBUG,
			 "Registers count exceeds the maximum limit, "
			 "reset register end to 0x%x",
			 *end);
	}
	...
	if (!(*ppos)) {
		...
		rc = __cam_cci_debug_cci_read_seq(cci_dbg->io_master_info, data,
						  start, *end,
						  cci_dbg->reg_addr_type,
						  cci_dbg->reg_data_type);
		if (rc) {
			goto exit;
		}
				     ...
				     enum camera_sensor_i2c_type data_type)
{
	...
	write_setting->reg_setting =
		vzalloc(sizeof(*write_setting->reg_setting));
					...
					size_t count, loff_t *ppos)
{
	...
	if ((cci_dbg->reg_addr_type == CAMERA_SENSOR_I2C_TYPE_WORD &&
	     reg > MAX_DEV_TYPE_WORD) ||
	    (reg > MAX_DEV_TYPE_BYTE &&
	...
	if ((cci_dbg->reg_data_type == CAMERA_SENSOR_I2C_TYPE_WORD &&
	     val > MAX_DEV_TYPE_WORD) ||
	    (val > MAX_DEV_TYPE_BYTE &&
	...
	rc = __cam_cci_debug_cci_write(cci_dbg->io_master_info, reg, val,
				       cci_dbg->reg_addr_type,
				       cci_dbg->reg_data_type);
	if (rc < 0) {
...
int cam_cci_dev_rename_debugfs_entry(void *cci_debug, char *device_name)
{
	int rc = 0;
	struct dentry *dbgfileptr = NULL;
	struct cam_cci_debug *cci_dbg = (struct cam_cci_debug *)cci_debug;
...
int cam_cci_dev_create_debugfs_entry(char *dev_name, uint32_t dev_index,
				     char *dev_type,
				     ...
				     void **cci_debug)
{
	...
	CAM_DBG(MI_DEBUG, "start to create debugfs for %s%u", dev_type,
		dev_index);
	...
	if (io_master_info->cci_client) {
		...
		if (!dbgfileptr) {
			CAM_ERR(MI_DEBUG,
				"%s: cci master debugfs dir lookup failed");
			rc = -ENOENT;
			goto end;
		}
		cci_dbg->parent_entry = dbgfileptr;

		snprintf(debugfs_name, DEBUGFS_NAME_MAX_SIZE, "%s%u", dev_type,
			 dev_index);
		dbgfileptr = debugfs_create_dir(debugfs_name, dbgfileptr);
		if (!dbgfileptr) {
			CAM_ERR(MI_DEBUG,
				"%s: cci device debugfs dir creation fail",
				dev_type);
			rc = -ENOENT;
			goto end;
		}
	} else if (io_master_info->qup_client) {
		snprintf(debugfs_name, DEBUGFS_NAME_MAX_SIZE, "qup");
		dbgfileptr = debugfs_lookup(debugfs_name, dbgfileptr);
		if (!dbgfileptr) {
			CAM_ERR(MI_DEBUG,
				"%s: qup master debugfs dir lookup failed");
			rc = -ENOENT;
			goto end;
		}
		cci_dbg->parent_entry = dbgfileptr;
		...
		if (!dbgfileptr) {
			CAM_ERR(MI_DEBUG,
				"%s: qup device debugfs dir creation fail",
				dev_type);
			rc = -ENOENT;
			goto end;
		}
		...
		CAM_WARN(
			MI_DEBUG,
			"%s: invaild i2c devices type debugfs dir creation fail",
			dev_type);
		rc = -ENOENT;
		goto end;
	}
	...
	dbgfileptr = debugfs_create_file("name", 0400, debugfs_device,
					 cci_dbg->name,
					 &cam_cci_debug_str_fops);

	dbgfileptr = debugfs_create_file("range", 0600, debugfs_device, cci_dbg,
					 &cam_cci_debug_range);

	dbgfileptr = debugfs_create_file("registers_func", 0600, debugfs_device,
					 cci_dbg, &cam_cci_debug_regs);

	dbgfileptr = debugfs_create_file("monitor_func", 0400, debugfs_device,
					 cci_dbg, &cam_cci_debug_monitor);

	snprintf(debugfs_name, DEBUGFS_NAME_MAX_SIZE, "property");
	dbgfileptr = debugfs_create_dir(debugfs_name, debugfs_device);
	if (!dbgfileptr) {
		CAM_ERR(MI_DEBUG,
			"%s: cci device property debugfs dir creation fail",
			dev_type);
		rc = -ENOENT;
		goto end;
	}
	...
	dbgfileptr = debugfs_create_file("index", 0400, debugfs_prop,
					 &cci_dbg->index,
					 &cam_cci_debug_val_readonly);

	dbgfileptr = debugfs_create_file("type", 0400, debugfs_prop,
					 cci_dbg->type,
					 &cam_cci_debug_str_fops);

	dbgfileptr = debugfs_create_file("reg_addr_type", 0600, debugfs_prop,
					 &cci_dbg->reg_addr_type,
					 &cam_cci_debug_addr_type);

	dbgfileptr = debugfs_create_file("reg_data_type", 0600, debugfs_prop,
					 &cci_dbg->reg_data_type,
					 &cam_cci_debug_data_type);

	dbgfileptr = debugfs_create_file("monitor_interval_ms", 0600,
					 debugfs_prop, &cci_dbg->interval_ms,
					 &cam_cci_debug_val);
	...
	*cci_debug = cci_dbg;
	...
	CAM_DBG(MI_DEBUG, "success to create debugfs for %s%u", dev_type,
		dev_index);
	return rc;
end:
	kfree(cci_dbg);
