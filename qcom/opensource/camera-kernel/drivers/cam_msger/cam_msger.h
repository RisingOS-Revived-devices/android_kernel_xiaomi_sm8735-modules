...
#define READER_DEATH_OMEN_TEMPLATE \
	"snd|%s|%d|{\"id\":5, \"what\":%d, \"time\":%llu, \"replay\":0, \"data\":{\"tid\":%d, \"pid\":%d, \"name\":%s}}"
...
#define switch_enable_and_exec(swh) if (msger_siwtches & swh)

#define switch_disable_and_exec(swh) if (!(msger_siwtches & swh))
...
typedef void (*ioctl_f)(struct file *filp, unsigned int cmd, unsigned long arg,
			u8 *hit);
typedef unsigned int (*poll_f)(struct file *file,
			       struct poll_table_struct *poll_table, u8 *hit);
