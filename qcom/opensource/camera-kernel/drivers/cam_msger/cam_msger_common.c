...
int find_pid_cmdline(char *cmdline)
{
	...
	for_each_process(task) {
		if (task == NULL || task->tasks.next == NULL ||
		    list_empty(&task->tasks))
			break;
		if (task->pid != task->tgid)
			continue;
		if (cmdline_len < TASK_COMM_LEN &&
		    strnlen(task->comm, TASK_COMM_LEN) > cmdline_len)
			continue;
	...
	for (idx = 0; idx < PID_ARRAY_SIZE; idx++) {
		if (pids[idx] > 0) {
			...
			if (!task)
				continue;
			tmp_cmdline =
				kstrdup_quotable_cmdline(task, GFP_KERNEL);
			m_pr(LTYPE_COC,
			     "[%s] tm_cmdline:%s cmdline:%s camdline_len:%d strncmp:%d\n",
			     __func__, tmp_cmdline, cmdline, cmdline_len,
			     strncmp(tmp_cmdline, cmdline, cmdline_len));
