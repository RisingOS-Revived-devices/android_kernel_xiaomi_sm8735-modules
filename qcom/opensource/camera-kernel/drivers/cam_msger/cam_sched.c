...
void cam_sched_ioctl(struct file *filp, unsigned int cmd, unsigned long arg,
		     u8 *hit)
{
...
int cam_sched_w_pids(struct file *fp, char *buff, int length, u8 *hit)
{
	...
	*hit = 0;
	...
	while ((token = strsep(&rest, SPLIT_DELIM)) != NULL) {
		m_pr(LTYPE_SCHED,
		     "[%s] idx:%d token:%s sizeof_token:%lu rest:%s \n",
		     __func__, idx, token, strnlen(token, CMDLINE_MAX_LEN),
		     rest);
		...
		if (idx == 2) {
			m_pr(LTYPE_SCHED, "[%s] token_len:%lu, %lu, %lu \n",
			     __func__, strlen(token), strlen(TISOT_ALLOW_STR),
			     strlen(TISOT_NRALLOW_STR));
			if (strnlen(token, TISOT_STR_LEN_MAX) >=
				    strlen(TISOT_ALLOW_STR) &&
			    strncmp(token, TISOT_ALLOW_STR,
				    strlen(TISOT_ALLOW_STR)) == 0) {
				tisot_flag = TISOT_ALLOW;
				goto __continue__;
			}
			if (strnlen(token, TISOT_STR_LEN_MAX) >=
				    strlen(TISOT_NRALLOW_STR) &&
			    strncmp(token, TISOT_NRALLOW_STR,
				    strlen(TISOT_NRALLOW_STR)) == 0) {
...
static bool effect_cpu_action(struct task_struct *p, int cpu, u8 *flag,
			      bool *ctl_proc, int *offset)
{
	bool ret = false;
	int i = 0;
	pid_t pids_tmp[MONITOR_MAX_PID] = { 0 };

	if (!p || p->mm == NULL || !cpu_active(cpu) || p->nr_cpus_allowed <= 2)
		return ret;
	...
	*offset = rcu_dereference(csdata)->offset;
	memcpy(&(pids_tmp[0]), &(rcu_dereference(csdata)->pids[0]),
	       sizeof(pid_t) * MONITOR_MAX_PID);
	rcu_read_unlock();
...
static void filter_cpu(struct task_struct *p, int prev_cpu, int *cpu,
		       bool ctl_proc, int flag, int offset)
{
	int cpuidx = -1;
	bool is_ctl = false;
	...
	m_pr(LTYPE_SCHED, "[%s] %s-%d cpu:%d ctl_proc:%d flag:%d offset:%d \n",
	     __func__, p->comm, p->pid, *cpu, ctl_proc, flag, offset);

	rcu_read_lock();
	is_ctl = cpumask_test_cpu(prev_cpu,
				  &(rcu_dereference(csdata)->ctl_cpus));
	cpumask_andnot(&(allowed_cpus), p->cpus_ptr,
		       &(rcu_dereference(csdata)->ctl_cpus));
	rcu_read_unlock();

	m_pr(LTYPE_SCHED, "[%s] %s-%d cpu:%d allowed_cpus:%*pbl\n", __func__,
	     p->comm, p->pid, *cpu, cpumask_pr_args(&(allowed_cpus)));
	...
	if (offset == 0 || (flag == TISOT_ALLOW && !ctl_proc) ||
	    (flag == TISOT_NEVERALLOW && ctl_proc)) {
		*cpu = cpuidx;
	}
}
static void xmi_rvh_is_cpu_allowed(void *unused, struct task_struct *p, int cpu,
				   bool *allowed)
{
	...
	if (effect_cpu_action(p, cpu, &flag, &ctl_proc, &offset)) {
		...
		*allowed = true;
		...
		m_pr(LTYPE_SCHED, "[%s] %s-%d cpu:%d allowed:%d\n", __func__,
		     p->comm, p->pid, cpu, *allowed);
	}
}
static void xmi_rvh_select_task_rq_fair(void *unused, struct task_struct *p,
...
static void xmi_can_migrate_task(void *unused, struct task_struct *p,
				 int dst_cpu, int *can_migrate)
{
	...
	if (effect_cpu_action(p, dst_cpu, &flag, &ctl_proc, &offset)) {
		m_pr(LTYPE_SCHED,
		     "[%s-begin] %s-%d dest_cpu:%d cam_migrate:%d ctl_proc:%d flag:%d\n",
		     __func__, p->comm, p->pid, dst_cpu, *can_migrate, ctl_proc,
		     flag);
		if (offset == 0) {
			*can_migrate = 0;
			goto __func_done_;
...
__func_done_:
		m_pr(LTYPE_SCHED,
		     "[%s-finish] %s-%d dest_cpu:%d cam_migrate:%d\n", __func__,
		     p->comm, p->pid, dst_cpu, *can_migrate);
	}
}
static void xmi_sched_newidle_balance(void *unused, struct rq *this_rq,
...
void cam_sched_init(void)
{
	...
	register_trace_android_rvh_select_task_rq_fair(
		xmi_rvh_select_task_rq_fair, NULL);
	register_trace_android_rvh_sched_newidle_balance(
		xmi_sched_newidle_balance, NULL);
