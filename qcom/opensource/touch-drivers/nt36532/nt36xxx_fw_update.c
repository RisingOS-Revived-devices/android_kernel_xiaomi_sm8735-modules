...
static int32_t nvt_bin_header_parser(const u8 *fwdata, size_t fwsize)
{
	...
	while (pos < end) {
		...
		if ((byte_to_word(&fwdata[pos + 4]) != 0) &&
		    (byte_to_word(&fwdata[pos + 8]) == 0)) {
			end = pos + 0x10;
			break;
		}
		pos += 0x10;
...
static void nvt_read_bld_hw_crc(void)
{
	...
	if (ts->bld_multi_header >= BLDMHEADER_SUPPORT) {
		...
		NVT_ERR("0x%02x 0x%02x, crc_done = %d, ilm_crc_flag = %d, dlm_crc_flag = %d\n",
			buf[1], buf[2], (buf[2] >> 3) & 0x01,
			(buf[1] >> 4) & 0x01, (buf[1] >> 5) & 0x01);
	} else {
		nvt_set_page(ts->mmap->BLD_ILM_DLM_CRC_ADDR);
		buf[0] = ts->mmap->BLD_ILM_DLM_CRC_ADDR & 0x7F;
		buf[1] = 0x00;
		...
		NVT_ERR("0x%02x, crc_done = %d, ilm_crc_flag = %d, dlm_crc_flag = %d\n",
			buf[1], (buf[1] >> 2) & 0x01, (buf[1] >> 0) & 0x01,
			(buf[1] >> 1) & 0x01);
	}
...
int32_t nvt_update_firmware(char *firmware_name)
{
	...
	if (ret) {
		NVT_ERR("[DIS-TF-TOUCH] update_firmware_request failed. (%d)\n",
			ret);
