...
#define MP_LIMIT_CSV_FILE \
	"MP_Setting_Criteria" /* Ex. MP_Setting_Criteria_5F86.csv*/
...
struct mp_criteria_item {
	const char *name;
	void *data;
	uint8_t type;
};

struct mp_criteria_item mp_criteria_item_table[] = {
	{ "Limit_Version:", &Limit_Version, TYPE_ITEM },
	{ "IC_X_CFG_SIZE:", &IC_X_CFG_SIZE, TYPE_ITEM },
	{ "IC_Y_CFG_SIZE:", &IC_Y_CFG_SIZE, TYPE_ITEM },
	{ "X_Channel:", &X_Channel, TYPE_ITEM },
	{ "Y_Channel:", &Y_Channel, TYPE_ITEM },
	{ "AIN_X:", &AIN_X, TYPE_AIN },
	{ "AIN_Y:", &AIN_Y, TYPE_AIN },
#if TOUCH_KEY_NUM > 0
	{ "IC_KEY_CFG_SIZE:", &IC_KEY_CFG_SIZE, TYPE_ITEM },
	{ "Key_Channel:", &Key_Channel, TYPE_ITEM },
	{ "AIN_KEY:", &AIN_KEY, TYPE_AIN },
#endif
	...
	{ "PS_Config_Lmt_FW_Rawdata_N:", &PS_Config_Lmt_FW_Rawdata_N,
	  TYPE_ARRAY },
	{ "PS_Config_Lmt_FW_CC_P:", &PS_Config_Lmt_FW_CC_P, TYPE_ARRAY },
	{ "PS_Config_Lmt_FW_CC_N:", &PS_Config_Lmt_FW_CC_N, TYPE_ARRAY },
	{ "PS_Config_Lmt_FW_Diff_P:", &PS_Config_Lmt_FW_Diff_P, TYPE_ARRAY },
	{ "PS_Config_Lmt_FW_Diff_N:", &PS_Config_Lmt_FW_Diff_N, TYPE_ARRAY },
	...
	{ "PS_Config_Lmt_PenRingX_FW_Diff_P:",
	  &PS_Config_Lmt_PenRingX_FW_Diff_P, TYPE_ARRAY_PEN_X },
	{ "PS_Config_Lmt_PenRingX_FW_Diff_N:",
	  &PS_Config_Lmt_PenRingX_FW_Diff_N, TYPE_ARRAY_PEN_X },
	{ "PS_Config_Lmt_PenRingY_FW_Diff_P:",
	  &PS_Config_Lmt_PenRingY_FW_Diff_P, TYPE_ARRAY_PEN_Y },
	{ "PS_Config_Lmt_PenRingY_FW_Diff_N:",
	  &PS_Config_Lmt_PenRingY_FW_Diff_N, TYPE_ARRAY_PEN_Y },
	{ "PS_Config_Diff_Test_Frame:", &PS_Config_Diff_Test_Frame, TYPE_ITEM },
};
...
static int32_t nvt_read_pen_raw(void)
#else /* #if TOUCH_THP_SUPPORT */
...
static int32_t nvt_read_fw_noise(int32_t *xdata)
{
	...
	nvt_read_mdata(ts->mmap->DIFF_PIPE1_ADDR,
		       ts->mmap->DIFF_BTN_PIPE1_ADDR);
	nvt_get_mdata(RawData_Diff_Max, &x_num, &y_num);
	nvt_read_mdata(ts->mmap->DIFF_PIPE0_ADDR,
		       ts->mmap->DIFF_BTN_PIPE0_ADDR);
	nvt_get_mdata(RawData_Diff_Min, &x_num, &y_num);
#else
	if (nvt_get_fw_pipe() == 0)
		nvt_read_mdata(ts->mmap->DIFF_PIPE0_ADDR,
			       ts->mmap->DIFF_BTN_PIPE0_ADDR);
...
void nvt_print_selftest_result(int32_t TestResult, uint8_t RecordResult[],
			       int32_t rawdata[], uint8_t x_len, uint8_t y_len)
{
	...
	case -1:
		...
		printk("ReadData:\n");
		for (i = 0; i < y_len; i++) {
			nvt_print_data_log_in_one_line(rawdata + i * x_len,
						       x_len);
			printk("\n");
		}
		break;
...
static int32_t c_show_selftest(struct seq_file *m, void *v)
{
	...
	nvt_mp_seq_printf(
		m, "FW Version: %d, NVT PID: 0x%04X, Limit Version: %d\n",
		fw_ver, nvt_pid, ts->limit_version);
	...
	if (ts->pen_support) {
		...
		print_selftest_result(m, TestResult_PenTipY_Raw,
				      RecordResult_PenTipY_Raw,
				      RawData_PenTipY_Raw, ts->x_gang_num,
				      ts->y_num);
		nvt_mp_seq_printf(m, "Pen Ring X Raw");
...
static int32_t nvt_selftest_open(struct inode *inode, struct file *file)
{
	...
	if (nvt_load_mp_setting_criteria_from_csv() < 0) {
		NVT_LOG("Fail load criteria from csv file, use default setting\n");
		//---Print Test Criteria---
		nvt_print_criteria();
	}
...
//---Pen FW Rawdata Test---
	...
	if (ts->ts_selftest_process_cmd) {
		if (nvt_set_extend_custom_cmd(0x19, 0x01) < 0) {
			NVT_ERR("nvt_set_extend_custom_cmd fail!");
		}
		if (nvt_set_extend_custom_cmd(0x01, 0x02) < 0) {
			NVT_ERR("nvt_set_extend_custom_cmd fail!");
		}
	}
...
int nvt_ic_self_test_short(void)
{
	...
	//---Download MP FW---
	if (nvt_update_firmware(MP_UPDATE_FIRMWARE_NAME) < 0) {
		NVT_ERR("[DIS-TF-TOUCH] update mp firmware failed!\n");
		goto short_failed_out;
	}
	...
	printk("TestResult_Short=%d\n", TestResult_Short);
	printk("Short Test");
	nvt_print_selftest_result(TestResult_Short, RecordResult_Short,
				  RawData_Short, X_Channel, Y_Channel);
...
int nvt_ic_self_test_open(void)
{
	...
	printk("TestResult_Open=%d\n", TestResult_Open);
	printk("Open Test");
	nvt_print_selftest_result(TestResult_Open, RecordResult_Open,
				  RawData_Open, X_Channel, Y_Channel);
...
int32_t parse_mp_setting_criteria_item(char **ptr, const char *name,
				       int32_t *item_value)
{
	char *tmp = NULL;
...
int32_t parse_mp_setting_ain_array(char **ptr, const char *name, uint8_t *array,
				   uint32_t size)
{
	char *tmp = NULL;
	int32_t i = 0;
	...
	tmp_buf = (char *)kzalloc(X_Y_MAX * 5 + 1, GFP_KERNEL);
	if (!tmp_buf) {
		NVT_ERR("kzalloc for tmp_buf failed!\n ");
		ret = -ENOMEM;
		goto failed;
	}
	...
	*ptr = tmp;
	...
	memset(tmp_buf, 0, (X_Y_MAX * 5 + 1) * sizeof(char));
	...
	while ((token = strsep(&tok_ptr, ", \t\r\0"))) {
		...
		if (!strcmp(token, "0xFF") || !strcmp(token, "0xff"))
			...
			array[i] = (uint8_t)simple_strtol(token, NULL, 10);
		//printk("%3d, ", array[i]);

		i++;
	}
	//printk("\n");
	// check if number equals to size
	if (i != size) {
		NVT_ERR("%s: load AIN failed!, i=%d, size=%d\n", __func__, i,
			size);
		ret = -EINVAL;
		goto failed;
	}
...
int32_t parse_mp_criteria_item_array(char **ptr, const char *name,
				     int32_t *item_array, int32_t x_ch,
				     int32_t y_ch)
{
	char *tmp = NULL;
	int32_t i = 0;
	...
	NVT_LOG("name=%s, x_ch=%d, y_ch=%d\n", name, x_ch, y_ch);
	if (x_ch == 0 || y_ch == 0) {
		return -EINVAL;
	}

	tmp_buf = (char *)kzalloc(X_Y_MAX * 7 + 1, GFP_KERNEL);
	if (!tmp_buf) {
		NVT_ERR("kzalloc for tmp_buf failed!\n ");
		ret = -ENOMEM;
		goto failed;
	...
	*ptr = tmp;
	...
	for (i = 0; i < y_ch; i++) {
		// walk thru this line
		goto_next_line(ptr);
		memset(tmp_buf, 0, (X_Y_MAX * 7 + 1) * sizeof(char));
		copy_this_line(tmp_buf, *ptr);
		offset = strlen(tmp_buf);
		tok_ptr = tmp_buf;
		j = 0;
		...
		//printk("\n");
		// check if j equals to x_ch
		if (j != x_ch) {
			NVT_ERR("%s: j not equal X size!, j=%d, x_ch=%d\n",
				__func__, j, x_ch);
			ret = -1;
			goto failed;
		}
	...
	while ((token = strsep(&tok_ptr, ", \t\r\0"))) {
		if (strlen(token) == 0)
			continue;
		item_array[y_ch * x_ch + i] =
			(int32_t)simple_strtol(token, NULL, 10);
		//printk("%5d, ", item_array[y_ch * x_ch + i]);
	...
	*ptr = *ptr + offset;
	...
	NVT_LOG("%s\n", name);
	nvt_print_data_array(item_array, x_ch, y_ch);
...
{
	int32_t ret = 0;
	unsigned char mpcriteria[64] = { 0 };
	char *fbufp = NULL; // buffer for content of file
	char *ptr = NULL;
	uint8_t item = 0;
	uint32_t len = 0;
	const struct firmware *f_file = NULL;

	NVT_LOG("++\n");

	snprintf(mpcriteria, sizeof(mpcriteria), "%s_%04X.csv",
		 MP_LIMIT_CSV_FILE, ts->nvt_pid);
	NVT_LOG("Parse mp criteria for file %s\n", mpcriteria);

	ret = request_firmware(&f_file, mpcriteria, &ts->client->dev);
	if (ret) {
		NVT_ERR("open %s failed, ret=%d\n", mpcriteria, ret);
		ret = -1;
		return ret;
	}

	fbufp = (char *)kzalloc(f_file->size + 2, GFP_KERNEL);
	if (!fbufp) {
	...
	memcpy(fbufp, f_file->data, f_file->size);

	fbufp[f_file->size] = '\0';
	fbufp[f_file->size + 1] = '\n';
	ptr = fbufp;
	while (ptr && (ptr < (fbufp + f_file->size))) {
		ptr = fbufp;
		switch (mp_criteria_item_table[item].type) {
		case TYPE_ITEM:
			if (parse_mp_setting_criteria_item(
				    &ptr, mp_criteria_item_table[item].name,
				    (int32_t *)mp_criteria_item_table[item]
					    .data) < 0) {
				ret = -1;
				goto exit_free;
			}
		...
		case TYPE_AIN:
			if (!strcmp(mp_criteria_item_table[item].name,
				    "AIN_X:"))
				len = IC_X_CFG_SIZE;
			else if (!strcmp(mp_criteria_item_table[item].name,
					 "AIN_Y:"))
				len = IC_Y_CFG_SIZE;
			else if (!strcmp(mp_criteria_item_table[item].name,
					 "AIN_KEY:"))
			...
			if (parse_mp_setting_ain_array(
				    &ptr, mp_criteria_item_table[item].name,
				    mp_criteria_item_table[item].data,
				    len) < 0) {
				ret = -1;
				goto exit_free;
			}
		...
		case TYPE_ARRAY_PEN_X:
			if (!ts->pen_support)
				break;
			if (parse_mp_criteria_item_array(
				    &ptr, mp_criteria_item_table[item].name,
				    mp_criteria_item_table[item].data,
				    ts->x_num, ts->y_gang_num) < 0) {
				ret = -1;
				goto exit_free;
			}
			break;

		case TYPE_ARRAY_PEN_Y:
			if (!ts->pen_support)
				break;
			if (parse_mp_criteria_item_array(
				    &ptr, mp_criteria_item_table[item].name,
				    mp_criteria_item_table[item].data,
				    ts->x_gang_num, ts->y_num) < 0) {
				ret = -1;
				goto exit_free;
			}
			break;
		...
		if (item == sizeof(mp_criteria_item_table) /
				    sizeof(struct mp_criteria_item)) {
			NVT_LOG("Load MP criteria and golden samples finished.\n");
			ts->limit_version = Limit_Version;
			break;
		}
	}
