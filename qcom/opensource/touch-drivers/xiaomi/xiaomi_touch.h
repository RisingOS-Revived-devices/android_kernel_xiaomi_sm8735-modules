...
#define IS_TOUCH_ID_INVALID(touch_id) \
...
#define LOG_ALWAYS(fmt, args...)                                      \
	do {                                                          \
		if (current_log_level >= MI_TP_LOG_ALWAYS) {          \
			pr_info(LOG_TAG "_A][%s:%d]: " fmt, __func__, \
				__LINE__, ##args);                    \
		}                                                     \
	} while (0)
...
#define LOG_ERROR(fmt, args...)                                       \
	do {                                                          \
		if (current_log_level >= MI_TP_LOG_ERROR) {           \
			pr_info(LOG_TAG "_E][%s:%d]: " fmt, __func__, \
				__LINE__, ##args);                    \
		}                                                     \
	} while (0)
...
#define LOG_WARNING(fmt, args...)                                     \
	do {                                                          \
		if (current_log_level >= MI_TP_LOG_WARNING) {         \
			pr_info(LOG_TAG "_W][%s:%d]: " fmt, __func__, \
				__LINE__, ##args);                    \
		}                                                     \
	} while (0)
...
#define LOG_INFO(fmt, args...)                                        \
	do {                                                          \
		if (current_log_level >= MI_TP_LOG_INFO) {            \
			pr_info(LOG_TAG "_I][%s:%d]: " fmt, __func__, \
				__LINE__, ##args);                    \
		}                                                     \
	} while (0)
...
#define LOG_DEBUG(fmt, args...)                                       \
	do {                                                          \
		if (current_log_level >= MI_TP_LOG_DEBUG) {           \
			pr_info(LOG_TAG "_D][%s:%d]: " fmt, __func__, \
				__LINE__, ##args);                    \
		}                                                     \
	} while (0)
...
#define LOG_VERBOSE(fmt, args...)                                     \
	do {                                                          \
		if (current_log_level >= MI_TP_LOG_VERBOSE) {         \
			pr_info(LOG_TAG "_V][%s:%d]: " fmt, __func__, \
				__LINE__, ##args);                    \
		}                                                     \
	} while (0)
...
#define XIAOMI_TOUCH_UTC_PRINT(tag)                                                         \
	do {                                                                                \
		...
		LOG_INFO(                                                                   \
			"%s[ktime:utc] [%llu.%llu : %ld-%02d-%02d %02d:%02d:%02d.%06lu]\n", \
			tag, time_ns / 1000000000,                                          \
			(time_ns % 1000000000) / 1000, tm.tm_year + 1900,                   \
...
typedef struct xiaomi_touch {
	...
	xiaomi_touch_driver_param_t
		xiaomi_touch_driver_param[MAX_TOUCH_PANEL_COUNT];
	int charging_status;
