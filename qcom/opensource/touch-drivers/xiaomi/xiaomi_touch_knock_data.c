...
void update_knock_data(u8 *buf, int size, int frame_id)
{
	...
	memcpy((unsigned char *)knock_data.raw_data, (unsigned char *)buf,
	       size);
#endif
...
static ssize_t knock_data_read(struct file *file, char __user *buf,
...
static ssize_t knock_data_write(struct file *file, const char __user *buf,
				size_t count, loff_t *pos)
{
	char temp_buf[6];
	int frame_count = 0;
...
end_set_value:
	...
	if (knock_data.need_frame_count_change_listener) {
		knock_data.need_frame_count_change_listener(
			knock_data.need_frame_count);
	}
...
static long knock_data_ioctl(struct file *file, unsigned int cmd,
			     unsigned long arg)
{
...
int knock_node_init(void)
{
	...
	if (!knock_data.raw_data) {
		...
		LOG_INFO("%s: %s: raw data addr:%lld, phy addr:%lld\n",
			 KNOCK_TAG, __func__,
			 (unsigned long)knock_data.raw_data,
			 (unsigned long)knock_data.phy_base);
	}
