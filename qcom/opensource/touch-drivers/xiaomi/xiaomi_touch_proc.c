...
static struct proc_dir_entry
	*tp_pde[MAX_TOUCH_PANEL_COUNT][PROC_COUNT_FOR_PANEL];
static int self_test_result[MAX_TOUCH_PANEL_COUNT];

static int proc_open(struct inode *inode, struct file *file)
{
	xiaomi_touch_driver_param_t *xiaomi_touch_driver_param =
		(xiaomi_touch_driver_param_t *)pde_data(inode);
	...
	xiaomi_touch_proc_data =
		kvzalloc_retry(sizeof(xiaomi_touch_proc_data_t), 3);
	...
	/* change private data */
	xiaomi_touch_proc_data->xiaomi_touch_driver_param =
	...
	if (!strncmp("tp_data_dump", name, 12)) {
		...
		xiaomi_touch_proc_data->tp_proc_result_buf =
			kvzalloc_retry(DUMP_DATA_BUF_SIZE, 3);
		...
		xiaomi_touch_proc_data->tp_proc_result_buf =
			kvzalloc_retry(NORMAL_DATA_BUF_SIZE, 3);
		if (!xiaomi_touch_proc_data->tp_proc_result_buf) {
			LOG_ERROR("alloc tp proc data memory error");
			return -1;
		}
...
{
	xiaomi_touch_proc_data_t *xiaomi_touch_proc_data =
		(xiaomi_touch_proc_data_t *)file->private_data;
	xiaomi_touch_driver_param_t *xiaomi_touch_driver_param =
		xiaomi_touch_proc_data->xiaomi_touch_driver_param;
	char *name = file->f_path.dentry->d_iname;
	s8 touch_id = -1;
...
static ssize_t proc_tp_read(struct file *file, char __user *buf, size_t count,
			    loff_t *pos)
{
	xiaomi_touch_proc_data_t *xiaomi_touch_proc_data =
		(xiaomi_touch_proc_data_t *)file->private_data;
	...
	if (!strncmp("tp_fw_version", name, 10)) {
		...
		n += snprintf(
			xiaomi_touch_proc_data->tp_proc_result_buf + n,
			NORMAL_DATA_BUF_SIZE - n, "fw version: %s\n",
			xiaomi_touch_driver_param->hardware_param.fw_version);
		n += snprintf(xiaomi_touch_proc_data->tp_proc_result_buf + n,
			      NORMAL_DATA_BUF_SIZE - n, "driver version: %s\n",
			      xiaomi_touch_driver_param->hardware_param
				      .driver_version);
		n += snprintf(xiaomi_touch_proc_data->tp_proc_result_buf + n,
			      NORMAL_DATA_BUF_SIZE - n, "hal version: %s\n",
			      xiaomi_touch_driver_param->hal_version);
		n += snprintf(xiaomi_touch_proc_data->tp_proc_result_buf + n,
			      NORMAL_DATA_BUF_SIZE - n,
		...
		u8 *lockdown_info =
			xiaomi_touch_driver_param->hardware_param.lockdown_info;
		LOG_DEBUG("read tp_lockdown_info");
		if (xiaomi_touch_driver_param->hardware_operation
			    .ic_get_lockdown_info) {
			ret = xiaomi_touch_driver_param->hardware_operation
				      .ic_get_lockdown_info(lockdown_info);
			if (ret < 0)
			...
			n = snprintf(
				xiaomi_touch_proc_data->tp_proc_result_buf + n,
				NORMAL_DATA_BUF_SIZE - n, "null");
		}
		xiaomi_touch_proc_data->tp_proc_result_length = n;
	} else if (!strncmp("tp_selftest", name, 11)) {
		LOG_DEBUG("read tp_selftest result %d",
			  self_test_result[touch_id]);
		n = snprintf(xiaomi_touch_proc_data->tp_proc_result_buf + n,
			     NORMAL_DATA_BUF_SIZE - n, "%d\n",
			     self_test_result[touch_id]);
		xiaomi_touch_proc_data->tp_proc_result_length = n;
		...
		if (xiaomi_touch_driver_param->hardware_operation
			    .ic_data_collect)
			xiaomi_touch_driver_param->hardware_operation
				.ic_data_collect(xiaomi_touch_proc_data
							 ->tp_proc_result_buf,
						 &n);
		xiaomi_touch_proc_data->tp_proc_result_length = n;
	}
...
copy_data_to_user:
	...
	LOG_DEBUG("result length %ld, valid length %d, count %zu",
		  xiaomi_touch_proc_data->tp_proc_result_length, valid_length,
		  count);
	if (valid_length &&
	    (ret = copy_to_user(
		     buf, xiaomi_touch_proc_data->tp_proc_result_buf + *pos,
		     valid_length))) {
...
static ssize_t proc_tp_write(struct file *file, const char __user *buf,
			     size_t count, loff_t *pos)
{
	...
	if (!strncmp("tp_fw_version", name, 10)) {
		...
		memset(xiaomi_touch_driver_param->hal_version, 0x00,
		       HAL_VERSION_LENGTH);
		if (temp_count >= HAL_VERSION_LENGTH) {
			LOG_ERROR(
				"write hal version length too long, current length is %zu",
				temp_count);
			temp_count = HAL_VERSION_LENGTH - 1;
		}
		if (copy_from_user(xiaomi_touch_driver_param->hal_version, buf,
				   temp_count)) {
			LOG_ERROR("copy hal version to buf has error");
			return -EFAULT;
		}
		LOG_INFO("hal version is %s",
			 xiaomi_touch_driver_param->hal_version);
		LOG_INFO("fw version is %s",
			 xiaomi_touch_driver_param->hardware_param.fw_version);
		LOG_INFO("driver version is %s",
			 xiaomi_touch_driver_param->hardware_param
				 .driver_version);
		...
		if (temp_count >= 64) {
			LOG_ERROR(
				"write self test cmd length too long, current length is %zu",
				temp_count);
			temp_count = 64 - 1;
		}
...
static struct proc_dir_entry *
create_proc_node(char *name, const struct proc_ops *proc_ops,
		 xiaomi_touch_driver_param_t *xiaomi_touch_driver_param)
{
	struct proc_dir_entry *pde = proc_create_data(
		name, 0644, NULL, proc_ops, xiaomi_touch_driver_param);
	kuid_t uid;
	kgid_t gid;
	...
	xiaomi_touch_driver_param_t *xiaomi_touch_driver_param)
{
	...
	tp_pde[touch_id][0] = create_proc_node(proc_name, &proc_tp_ops,
					       xiaomi_touch_driver_param);

	memset(proc_name, 0, 64);
	snprintf(proc_name, 64, "tp_lockdown_info%s", name_suffix);
	tp_pde[touch_id][1] = create_proc_node(proc_name, &proc_tp_ops,
					       xiaomi_touch_driver_param);

	memset(proc_name, 0, 64);
	snprintf(proc_name, 64, "tp_selftest%s", name_suffix);
	tp_pde[touch_id][2] = create_proc_node(proc_name, &proc_tp_ops,
					       xiaomi_touch_driver_param);

	memset(proc_name, 0, 64);
	snprintf(proc_name, 64, "tp_data_dump%s", name_suffix);
	tp_pde[touch_id][3] = create_proc_node(proc_name, &proc_tp_ops,
					       xiaomi_touch_driver_param);
