...
xiaomi_touch_data_t *get_xiaomi_touch_data(s8 touch_id)
{
	if (IS_TOUCH_ID_INVALID(touch_id)) {
		...
		LOG_ERROR("panel in touch id %d hasn't register, return!",
			  touch_id);
		return NULL;
	}
...
void notify_xiaomi_touch(xiaomi_touch_data_t *xiaomi_touch_data,
			 enum poll_notify_type type)
{
	private_data_t *client_private_data = NULL;
	if (!xiaomi_touch_data)
		return;
	spin_lock(&xiaomi_touch_data->private_data_lock);
	list_for_each_entry_rcu(client_private_data,
				&xiaomi_touch_data->private_data_list, node) {
		LOG_VERBOSE(
			"notify xiaomi-touch data update, client private data is %p",
			client_private_data);
		wake_up_all(&client_private_data->poll_wait_queue_head);
		if (type == COMMON_DATA_NOTIFY) {
			wake_up_all(&client_private_data
					     ->poll_wait_queue_head_for_cmd);
		} else if (type == FRAME_DATA_NOTIFY) {
			wake_up_all(&client_private_data
					     ->poll_wait_queue_head_for_frame);
		} else if (type == RAW_DATA_NOTIFY) {
			wake_up_all(&client_private_data
					     ->poll_wait_queue_head_for_raw);
		}
	}
	spin_unlock(&xiaomi_touch_data->private_data_lock);
}

void add_common_data_to_buf(s8 touch_id, enum common_data_cmd cmd,
			    enum common_data_mode mode, int length, int *data)
{
	...
	LOG_DEBUG("add touch id %d common mode: %d to buffer:%d", touch_id,
		  mode, atomic_read(&xiaomi_touch_data->common_data_buf_index));
	mutex_lock(&xiaomi_touch_data->common_data_buf_lock);
	common_data = &xiaomi_touch_data->common_data_buf[atomic_read(
		&xiaomi_touch_data->common_data_buf_index)];
	...
	if (atomic_read(&xiaomi_touch_data->common_data_buf_index) >=
...
void *get_raw_data_base(s8 touch_id)
{
	...
	if (!xiaomi_touch_data->frame_data_mmap_base) {
		LOG_ERROR(
			"touch id %d copy data failed, xiaomi_touch_data %p, base %p",
			touch_id, xiaomi_touch_data,
			xiaomi_touch_data->frame_data_mmap_base);
		return NULL;
	}

	base = xiaomi_touch_data->frame_data_mmap_base +
	       atomic_read(&xiaomi_touch_data->frame_data_buf_index) *
		       xiaomi_touch_data->frame_data_size;
	return base;
}
...
void notify_raw_data_update(s8 touch_id)
{
	...
	if (atomic_read(&xiaomi_touch_data->frame_data_buf_index) >=
	    xiaomi_touch_data->frame_data_buf_size)
...
static int xiaomi_touch_temp_thread_func(void *data)
{
	...
	while (!kthread_should_stop()) {
		wait_event_interruptible(
			xiaomi_touch_data->temp_detect_wait_queue,
		...
		if (atomic_read(&xiaomi_touch_data->temp_detect_ready)) {
			...
			cur_temp = (cur_temp0 + 5) /
				   10; // Rounding, in degrees Celsius
			if (abs(cur_temp0) < INVAILD_TEMPERATURE &&
			    xiaomi_touch_driver_param->hardware_operation
				    .set_thermal_temp &&
			    abs(cur_temp - last_temp) >=
				    xiaomi_touch_driver_param->hardware_param
					    .temp_change_value) {
				xiaomi_touch_driver_param->hardware_operation
			...
			LOG_DEBUG("cur_temp0:%d, last_temp:%d, sleep %dms",
				  cur_temp0, last_temp, temp_n * 1000);
			msleep(temp_n * 1000);
		}
	}
	LOG_INFO("exit");
			 ...
			 hardware_operation_t *hardware_operation)
{
	...
	if (xiaomi_touch.panel_register_mask & (1 << touch_id)) {
		LOG_ERROR("touch id %d has register, stop register again!",
			  touch_id);
		return -1;
	}
	...
	if (hardware_param) {
		xiaomi_touch_driver_param =
			&xiaomi_touch.xiaomi_touch_driver_param[touch_id];
		xiaomi_touch_driver_param->touch_id = touch_id;
		memcpy(&xiaomi_touch_driver_param->hardware_param,
		       hardware_param, sizeof(hardware_param_t));
	} else {
		LOG_ERROR("param is null");
		return -1;
	}
	if (hardware_operation)
		memcpy(&xiaomi_touch_driver_param->hardware_operation,
		       hardware_operation, sizeof(hardware_operation_t));
	...
	xiaomi_touch_data->frame_data_size =
		hardware_param->frame_data_page_size * PAGE_SIZE;
	xiaomi_touch_data->frame_data_buf_size =
		hardware_param->frame_data_buf_size;
	xiaomi_touch_data->raw_data_size =
		hardware_param->raw_data_page_size * PAGE_SIZE;
	xiaomi_touch_data->raw_data_buf_size =
	...
	alloc_size = xiaomi_touch_data->frame_data_size *
		     xiaomi_touch_data->frame_data_buf_size;
	LOG_DEBUG(
		"alloc size = %d, frame data size %d, frame data page size %d, frame data buf size %d",
		alloc_size, xiaomi_touch_data->frame_data_size,
		hardware_param->frame_data_page_size,
		hardware_param->frame_data_buf_size);
	if (alloc_size) {
		xiaomi_touch_data->frame_data_mmap_base =
			kzalloc_retry(alloc_size, 3);
		if (!xiaomi_touch_data->frame_data_mmap_base) {
			LOG_ERROR("touch id %d alloc frame data memory failed!",
				  touch_id);
			return -1;
		}
		LOG_DEBUG("frame data base %p, size %u",
			  xiaomi_touch_data->frame_data_mmap_base, alloc_size);
		xiaomi_touch_data->frame_data_mmap_phy_base =
			virt_to_phys(xiaomi_touch_data->frame_data_mmap_base);
	}
	...
	LOG_DEBUG(
		"alloc size = %d, raw data size %d, raw data page size %d, raw data buf size %d",
		alloc_size, xiaomi_touch_data->raw_data_size,
		hardware_param->raw_data_page_size,
		hardware_param->raw_data_buf_size);
	if (alloc_size) {
		xiaomi_touch_data->raw_data_mmap_base =
			kzalloc_retry(alloc_size, 3);
		if (!xiaomi_touch_data->raw_data_mmap_base) {
			LOG_ERROR("touch id %d alloc raw data memory failed!",
				  touch_id);
			return -1;
		}
		LOG_DEBUG("raw data base %p, size %u",
			  xiaomi_touch_data->raw_data_mmap_base, alloc_size);
		xiaomi_touch_data->raw_data_mmap_phy_base =
			virt_to_phys(xiaomi_touch_data->raw_data_mmap_base);
	}

	xiaomi_touch_data->event_wq =
		alloc_workqueue("xiaomi-touch-event-queue",
	...
	xiaomi_touch_data->poll_data =
		kzalloc_retry(sizeof(htc_ic_polldata_t), 3);
	...
	if (hardware_operation && hardware_param) {
		if (hardware_operation->ic_get_lockdown_info) {
			hardware_operation->ic_get_lockdown_info(
				hardware_param->lockdown_info);
		}
		LOG_INFO(
			"x_resolution=%d, y_resolution=%d, rx_num=%d, tx_num=%d, "
			...
			hardware_param->y_resolution, hardware_param->rx_num,
			hardware_param->tx_num,
			...
			hardware_param->lockdown_info[0],
			hardware_param->lockdown_info[1],
			hardware_param->lockdown_info[2],
			hardware_param->lockdown_info[3],
			hardware_param->lockdown_info[4],
			hardware_param->lockdown_info[5],
			hardware_param->lockdown_info[6],
			hardware_param->lockdown_info[7]);
	}
	/* init touch mode */
	...
	LOG_INFO("current panel_register_mask is %d",
		 xiaomi_touch.panel_register_mask);
	...
	if (hardware_operation && hardware_operation->set_thermal_temp) {
		...
		xiaomi_touch_temp_thread =
			kthread_create(xiaomi_touch_temp_thread_func,
				       (void *)&xiaomi_touch_data->touch_id,
				       "xiaomi_touch_temp_thread");
...
void unregister_touch_panel(s8 touch_id)
{
	...
	xiaomi_touch_driver_param_t *xiaomi_touch_driver_param = NULL;
	if (!(xiaomi_touch.panel_register_mask & (1 << touch_id))) {
		LOG_ERROR("touch id %d didn't register, break unregister!",
			  touch_id);
		return;
	}
	...
	memset(&xiaomi_touch_driver_param->hardware_param, 0,
	       sizeof(hardware_param_t));
	memset(&xiaomi_touch_driver_param->hardware_operation, 0,
	       sizeof(hardware_operation_t));
...
{
	xiaomi_touch_data_t *xiaomi_touch_data =
		container_of(work, xiaomi_touch_data_t, resume_work);
	...
	if (xiaomi_touch_driver_param->hardware_operation.ic_set_charge_state) {
		xiaomi_touch_driver_param->hardware_operation
			.ic_set_charge_state(xiaomi_touch.charging_status);
	}
...
{
	xiaomi_touch_data_t *xiaomi_touch_data =
		container_of(work, xiaomi_touch_data_t, suspend_work);
	s8 touch_id = xiaomi_touch_data->touch_id;
	xiaomi_touch_driver_param_t *xiaomi_touch_driver_param =
		get_xiaomi_touch_driver_param(touch_id);
	int value = 1;
...
static void xiaomi_drm_panel_notifier_callback(
	...
	struct xring_panel_event_notification *notification, void *client_data)
{
	...
	if (!notification || IS_TOUCH_ID_INVALID(touch_id)) {
		LOG_ERROR("Invalid data. notification %p, touch_id %ld",
			  notification, touch_id);
		return;
	}
	...
	struct panel_event_notification *notification, void *client_data)
{
	...
	case DRM_PANEL_EVENT_FPS_CHANGE:
		break;
	default:
		LOG_ERROR("notification serviced :%d",
			  notification->notif_type);
		break;
	}
	if (notification->notif_type < 4)
...
{
	xiaomi_touch_data_t *xiaomi_touch_data = container_of(
		work, xiaomi_touch_data_t, panel_notifier_register_work.work);
	...
	count = of_count_phandle_with_args(xiaomi_touch_data->dev->of_node,
					   property_name, NULL);
	for (i = 0; i < count; i++) {
		node = of_parse_phandle(xiaomi_touch_data->dev->of_node,
					property_name, i);
	...
	if (!panel) {
		...
		if (check_count++ < 5)
			schedule_delayed_work(
				&xiaomi_touch_data->panel_notifier_register_work,
...
void schedule_resume_suspend_work(s8 touch_id, bool resume_work)
{
	...
	flush_workqueue(xiaomi_touch_data->event_wq);
	if (resume_work) {
		queue_work_result = queue_work(xiaomi_touch_data->event_wq,
					       &xiaomi_touch_data->resume_work);
	} else {
		queue_work_result =
			queue_work(xiaomi_touch_data->event_wq,
				   &xiaomi_touch_data->suspend_work);
	}

	if (!queue_work_result) {
		LOG_ERROR("queue %s work failed, retry",
			  resume_work ? "resume" : "suspend");
	}
				    ...
				    int panel_event_notifier_client)
{
	...
	xiaomi_touch_data->panel_event_notifier_client =
		panel_event_notifier_client;
	INIT_DELAYED_WORK(&xiaomi_touch_data->panel_notifier_register_work,
			  xiaomi_register_panel_notifier_work);
	...
	schedule_delayed_work(&xiaomi_touch_data->panel_notifier_register_work,
			      msecs_to_jiffies(0));
#endif
}
...
static void xiaomi_power_supply_work(struct work_struct *work)
{
	xiaomi_touch_t *xiaomi_touch =
		container_of(work, xiaomi_touch_t, power_supply_work);
	int charging_status;
	int i = 0;
	xiaomi_touch_driver_param_t *xiaomi_touch_driver_param;

	charging_status = xiaomi_get_charging_status();
	if (charging_status != xiaomi_touch->charging_status ||
	    xiaomi_touch->charging_status < 0) {
		...
		for (i = 0; i < MAX_TOUCH_PANEL_COUNT; i++) {
			...
			if (xiaomi_touch_driver_param->hardware_operation
				    .ic_set_charge_state) {
				xiaomi_touch_driver_param->hardware_operation
					.ic_set_charge_state(charging_status);
			}
		}
	}
}

static int xiaomi_power_supply_notifier_callback(struct notifier_block *nb,
...
static void xiaomi_register_power_supply_event(xiaomi_touch_t *xiaomi_touch)
{
	...
	xiaomi_touch->power_supply_notifier.notifier_call =
		xiaomi_power_supply_notifier_callback;
	retval =
		power_supply_reg_notifier(&xiaomi_touch->power_supply_notifier);
	if (retval < 0) {
		LOG_ERROR("error:%d\n", retval);
		return;
	}
...
void nfc_to_touch_event(s8 touch_id, u8 val)
{
	...
	if (!xiaomi_touch_driver_param) {
		LOG_INFO("Touch panel  id %d hasn't register, return!",
			 touch_id);
		return;
	}
	if (xiaomi_touch_driver_param->hardware_operation.set_nfc_to_touch_event)
		xiaomi_touch_driver_param->hardware_operation
			.set_nfc_to_touch_event(val);
	else
