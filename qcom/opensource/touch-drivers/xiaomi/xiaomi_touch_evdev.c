...
static void last_touch_events_collect(struct input_handle *handle,
				      ...
				      int value)
{
	...
	if (code == ABS_MT_TRACKING_ID) {
		...
		if (handle->dev->name != NULL) {
			strncpy(last_touch_events
					.touch_event_buf[last_touch_events.head]
					.name,
				handle->dev->name, MAX_NAME_LENGTH);
		} else {
			strncpy(last_touch_events
					.touch_event_buf[last_touch_events.head]
					.name,
				DEFAULT_INPUT_DEVICE_NAME, MAX_NAME_LENGTH);
		}
		last_touch_events.touch_event_buf[last_touch_events.head]
			.name[MAX_NAME_LENGTH - 1] = '\0';
		last_touch_events.touch_event_buf[last_touch_events.head].state =
			!!state ? EVENT_DOWN : EVENT_UP;
		last_touch_events.touch_event_buf[last_touch_events.head].slot =
			slot;
		ktime_get_real_ts64(
			&last_touch_events
				 .touch_event_buf[last_touch_events.head]
...
static void print_version_info_in_last_touch_events(struct seq_file *m)
{
	...
	for (touch_id = 0; touch_id < MAX_TOUCH_PANEL_COUNT; touch_id++) {
		...
		if (strlen(xiaomi_touch_driver_param->hardware_param
				   .fw_version) == 0 &&
...
static int32_t event_show(struct seq_file *m, void *v)
{
	...
	seq_printf(m,
		   "%d-%02d-%02d %02d:%02d:%02d.%09lu UTC Finger %s (%2d) %s\n",
...
void add_input_event_timeline_before_event_time(int type, u64 frame_count,
						s64 start_time, s64 end_time)
{
	...
	if (!input_event_time_line_base)
		return;
	input_event_time_line =
		&input_event_time_line_base[input_event_time_line_index];
	if (type == 0) { /* irq start and end time */
		if (input_event_time_line->noEventTimeCount >=
		    NO_EVENT_TIME_BUF_SIZE) {
			for (i = 1; i < NO_EVENT_TIME_BUF_SIZE; i++) {
				input_event_time_line
					->startInterruptTime[i - 1] =
					input_event_time_line
						->startInterruptTime[i];
				input_event_time_line->endInterruptTime[i - 1] =
					input_event_time_line
						->endInterruptTime[i];
				input_event_time_line->startThpTime[i - 1] =
					input_event_time_line->startThpTime[i];
				input_event_time_line->endThpTime[i - 1] =
					input_event_time_line->endThpTime[i];
			}
...
static void add_input_event_event_time(struct input_handle *handle)
{
	...
	if (unlikely(event_time == 0)) {
		LOG_ERROR(
			"xiaomi handler get time is 0, please reboot and retry!");
		return;
	}
	...
	if (input_event_time_line->noEventTimeCount >= NO_EVENT_TIME_BUF_SIZE) {
		LOG_ERROR(
			"no point irq count is more than buf, maybe lose data!");
	}
	...
	LOG_INFO("add event time %lld in index %d", event_time,
...
bool init_input_event_timeline(void)
{
	...
	xiaomi_touch->input_event_time_line_mmap_base =
		kzalloc_retry(PAGE_SIZE * 45, 3);
	if (!xiaomi_touch->input_event_time_line_mmap_base) {
		LOG_ERROR("alloc input event time line memory failed!");
		return false;
	} else {
		LOG_INFO("init input event time line base %p",
			 xiaomi_touch->input_event_time_line_mmap_base);
		input_event_time_line_base =
			(input_event_time_line_t
				 *)(xiaomi_touch
					    ->input_event_time_line_mmap_base);
		xiaomi_touch->input_event_time_line_phy_base = virt_to_phys(
			xiaomi_touch->input_event_time_line_mmap_base);
	}
	return true;
}
...
int xiaomi_touch_evdev_init(xiaomi_touch_t *_xiaomi_touch)
{
	...
	if (!last_touch_events_pde) {
		last_touch_events_pde =
			proc_create_seq("last_touch_events", 0644, NULL,
					&last_touch_events_seq_ops);
		if (!last_touch_events_pde) {
			LOG_ERROR(
				"last_touch_events_proc create has error, exit!");
			return -1;
		}
