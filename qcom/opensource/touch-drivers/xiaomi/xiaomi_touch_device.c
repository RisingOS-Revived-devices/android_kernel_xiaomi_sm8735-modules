...
struct input_dev *register_xiaomi_input_dev(s8 touch_id, int max_x, int max_y,
					    ic_product_code code)
{
	int ret;
	...
	snprintf(name[touch_id], DEVICE_NAME_LEN_MAX, "Xiaomi_Touch_Input_%d",
		 touch_id);
	snprintf(phys[touch_id], DEVICE_NAME_LEN_MAX, "Xiaomi_Touch_Device_%d",
		 touch_id);
	LOG_INFO("xiaomi allocat !!!,the device is %s, %s, %d, %d",
		 name[touch_id], phys[touch_id], XIAOMI_TOUCH_VENDOR_ID, code);
...
struct input_dev *register_xiaomi_stylus_input_dev(s8 touch_id, int max_x,
						   int max_y,
						   ic_product_code code)
{
	int ret;
	...
	snprintf(name[touch_id], DEVICE_NAME_LEN_MAX,
		 "Xiaomi_Touch_Stylus_Input_%d", touch_id);
	snprintf(phys[touch_id], DEVICE_NAME_LEN_MAX,
		 "Xiaomi_Touch_Stylus_Device_%d", touch_id);
	LOG_INFO("xiaomi allocat !!! the device is %s, %s", name[touch_id],
		 phys[touch_id]);
...
static struct input_dev *report_finger_event(s8 touch_id,
					     hal_report_piont_t *report_point)
{
	static u16 finger_report_point_bitmap = 0;
	struct input_dev *input_dev = input_devs[touch_id];
	if (input_dev == NULL) {
		LOG_INFO("touch id %d input device is empty ,fail to report!",
			 touch_id);
		return NULL;
	}
	if (report_point->finger.point_action != ACTION_UP) {
		...
		input_mt_slot(input_dev,
			      report_point->finger.prop[FINGER_SLOT]);
		input_mt_report_slot_state(input_dev, MT_TOOL_FINGER, 1);
		input_report_abs(input_dev, ABS_MT_POSITION_X,
				 report_point->finger.x);
		input_report_abs(input_dev, ABS_MT_POSITION_Y,
				 report_point->finger.y);
		input_report_abs(input_dev, ABS_MT_TOUCH_MAJOR,
				 report_point->finger.prop[FINGER_MAJOR]);
		input_report_abs(input_dev, ABS_MT_TOUCH_MINOR,
				 report_point->finger.prop[FINGER_MINOR]);
		__set_bit(report_point->finger.prop[FINGER_SLOT],
			  (unsigned long *)&finger_report_point_bitmap);
	} else {
		input_mt_slot(input_dev,
			      report_point->finger.prop[FINGER_SLOT]);
		input_mt_report_slot_state(input_dev, MT_TOOL_FINGER, 0);
		__clear_bit(report_point->finger.prop[FINGER_SLOT],
			    (unsigned long *)&finger_report_point_bitmap);
	}
...
static struct input_dev *report_stylus_event(s8 touch_id,
					     hal_report_piont_t *report_point)
{
	...
	if (report_point->stylus.point_action != ACTION_UP) {
		...
		input_report_abs(stylus_input_dev, ABS_X,
				 report_point->stylus.x);
		input_report_abs(stylus_input_dev, ABS_Y,
				 report_point->stylus.y);
		input_report_abs(stylus_input_dev, ABS_PRESSURE,
				 report_point->stylus.prop[STYLUS_PRESSURE]);
		input_report_abs(stylus_input_dev, ABS_DISTANCE,
				 report_point->stylus.prop[STYLUS_DISTANCE]);
		input_report_abs(stylus_input_dev, ABS_TILT_X,
				 report_point->stylus.prop[STYLUS_TILT_X]);
		input_report_abs(stylus_input_dev, ABS_TILT_Y,
				 report_point->stylus.prop[STYLUS_TILT_Y]);
	} else {
...
int report_touch_event(s8 touch_id, u8 event_count)
{
	struct input_dev *input_dev = NULL;
	struct input_dev *stylus_input_dev = NULL;
	hal_report_piont_t *event =
		(hal_report_piont_t *)report_point_mmap_base;
	int index = 0;
	...
	for (index = 0; index < event_count; index++) {
		if (event[index].input_style == INPUT_STYLE_FINGER) {
			input_dev =
				report_finger_event(touch_id, &event[index]);
		} else if (event[index].input_style == INPUT_STYLE_STYLUS) {
			stylus_input_dev =
				report_stylus_event(touch_id, &event[index]);
		} else {
			LOG_ERROR("input style %d is invalid",
				  event[index].input_style);
		}
		cond_resched();
	}
	if (input_dev != NULL) {
