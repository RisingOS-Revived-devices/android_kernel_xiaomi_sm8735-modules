...
unsigned int soc_sia81xx_get_aux_num(struct platform_device *pdev)
{
	int ret = 0;
	...
	ret = of_property_read_u32(pdev->dev.of_node, "si,sia81xx-max-num",
				   &max_dev_num);
	if (0 != ret) {
...
unsigned int soc_sia81xx_get_codec_conf_num(struct platform_device *pdev)
{
	return soc_sia81xx_get_aux_num(pdev);
}
EXPORT_SYMBOL(soc_sia81xx_get_codec_conf_num);

int soc_sia81xx_init(struct platform_device *pdev,
		     ...
		     struct snd_soc_codec_conf *codec_conf, u32 conf_num)
{
	...
	for (i = 0; i < dev_num; i++) {
		...
		ret = of_property_read_string_index(
			pdev->dev.of_node, "si,sia81xx-aux-devs-prefix", i,
			&dev_name_prefix);
		if (0 != ret) {
		...
		aux_dev_name = devm_kstrdup(
			&pdev->dev, (const char *)dev_name(&sia81xx->pdev->dev),
			GFP_KERNEL);
...
int soc_sia81xx_deinit(struct platform_device *pdev,
		       struct snd_soc_aux_dev *aux_dev, u32 aux_num,
		       struct snd_soc_codec_conf *codec_conf, u32 conf_num)
{
	int i = 0;
...
int soc_aux_deinit_only_sia81xx(struct platform_device *pdev,
				struct snd_soc_card *card)
{
	if (NULL == pdev) {
		pr_err("[  err][%s] : NULL == pdev \n", __func__);
	...
	soc_sia81xx_deinit(pdev, card->aux_dev, card->num_aux_devs,
			   card->codec_conf, card->num_configs);
...
int soc_sia91xx_init(struct platform_device *pdev,
		     struct snd_soc_codec_conf *codec_conf, u32 conf_num)
{
	int ret, i;
	u32 dev_num = 0, prefix_num = 0;
...
int soc_codec_conf_sia91xx(struct platform_device *pdev,
			   struct snd_soc_card *card)
{
	int ret = 0;
	u32 conf_num = 0;
...
int soc_codec_conf_sipa(struct platform_device *pdev, struct snd_soc_card *card)
{
	int ret = 0;
	u32 is_digital_pa = 0;

	ret = of_property_read_u32(pdev->dev.of_node, "si,is_digital_pa",
				   &is_digital_pa);
	if (0 != ret) {
