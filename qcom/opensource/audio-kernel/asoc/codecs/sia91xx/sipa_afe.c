...
static int afe_port_to_apr_port(uint16_t afe_port)
{
	/* range: 0x10 ~ APR_MAX_PORTS�� to avoid the port : 0x00 */
	return (afe_port % (APR_MAX_PORTS - 0x10)) + 0x10;
}

static int sipa_fill_apr_hdr(struct apr_hdr *apr_hdr, uint32_t port,
			     uint32_t opcode, uint32_t apr_msg_size)
{
	...
	apr_hdr->hdr_field = APR_HDR_FIELD(
...
static int32_t sipa_afe_callback(struct apr_client_data *data, void *priv)
{
	SIPA_AFE *afe = NULL;
	...
	case AFE_SERVICE_CMDRSP_SHARED_MEM_MAP_REGIONS: {
		struct afe_service_cmdrsp_shared_mem_map_regions *payload =
			NULL;

		if (!data->payload_size) {
			pr_err("[  err][%s] %s: invalid payload_size = %d "
			       "with opcode = 0x%08x \n",
			       LOG_FLAG, __func__, data->payload_size,
			       data->opcode);
			break;
		}

		payload = (struct afe_service_cmdrsp_shared_mem_map_regions *)
				  data->payload;
...
static int sipa_apr_send(SIPA_AFE *afe, void *cmd)
{
	int ret = 0;
	...
	if (!(wait_event_timeout(afe->wait, (atomic_read(&afe->state) == 0),
...
static int check_apr(SIPA_AFE *afe)
{
	int ret = 0;
...
static int check_cal_block(SIPA_AFE *afe)
{
	int ret = 0;
...
static inline int chack_afe_validity(SIPA_AFE *afe)
{
	if (NULL == afe) {
...
static void sipa_write_param_to_payload(afe_cmd_sipa_param_payload *payload,
					uint32_t module_id, uint32_t param_id,
					uint32_t param_size, uint8_t *param)
{
...
static int sipa_read_param_from_payload(afe_cmd_sipa_param_payload *payload,
					uint32_t module_id, uint32_t param_id,
					uint32_t param_size, uint8_t *param)
{
...
static int sipa_apr_cmd_memory_unmap(SIPA_AFE *afe)
{
	int ret = 0;
...
static SIPA_AFE *find_sipa_afe_list(uint16_t afe_port_id)
{
	SIPA_AFE *afe = NULL;
...
static void add_sipa_afe_list(SIPA_AFE *afe)
{
	if (NULL == afe) {
		pr_err("[  err][%s] %s: NULL == afe \n", LOG_FLAG, __func__);
		return;
...
static void del_sipa_afe_list(SIPA_AFE *afe)
{
	if (NULL == afe) {
		pr_err("[  err][%s] %s: NULL == afe \n", LOG_FLAG, __func__);
		return;
	...
	pr_debug("[debug][%s] %s: del fe port id : %u, count = %u \n", LOG_FLAG,
		 __func__, afe->afe_port_id, sipa_afe_list_count());
...
int sipa_afe_send(unsigned long afe_handle, uint32_t module_id,
		  uint32_t param_id, uint32_t param_size, uint8_t *buf)
{
	int ret = 0;
	...
	sipa_write_param_to_payload(
		...
		module_id, param_id, param_size, buf);
...
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(4, 14, 0))
	/* fill cmd head */
	sipa_fill_apr_hdr((struct apr_hdr *)(&apr_cmd.apr_hdr),
			  afe_port_to_apr_port(afe->afe_port_id),
			  AFE_PORT_CMD_SET_PARAM_V2, sizeof(apr_cmd));

	/* fill cmd entity */
	apr_cmd.port_id = afe->afe_port_id;
	apr_cmd.payload_size = param_size + sizeof(afe_cmd_sipa_param_payload);
...
#else
	/* fill cmd head */
	sipa_fill_apr_hdr((struct apr_hdr *)(&apr_cmd.hdr),
			  afe_port_to_apr_port(afe->afe_port_id),
			  AFE_PORT_CMD_SET_PARAM_V2, sizeof(apr_cmd));
	...
	apr_cmd.param.payload_address_lsw = lower_32_bits(afe->cal_block.paddr);
...
int sipa_afe_read(unsigned long afe_handle, uint32_t module_id,
		  uint32_t param_id, uint32_t param_size, uint8_t *buf)
{
	int ret = 0;
	...
	sipa_fill_apr_hdr((struct apr_hdr *)(&apr_cmd.apr_hdr),
			  afe_port_to_apr_port(afe->afe_port_id),
			  AFE_PORT_CMD_GET_PARAM_V2, sizeof(apr_cmd));
...
unsigned long sipa_afe_open(uint32_t afe_prot_id)
{
	SIPA_AFE *afe;
...
int sipa_afe_close(unsigned long afe_handle)
{
	int ret = 0;
	SIPA_AFE *afe = (SIPA_AFE *)afe_handle;
	...
	if ((NULL != afe->cal_block.ion_client) ||
		...
		ret = msm_audio_ion_free(afe->cal_block.ion_client,
					 afe->cal_block.ion_handle);
		if (0 != ret) {
			pr_err("[  err][%s] %s: ion free error, close failed !! "
			       "ret = %d \n",
			       LOG_FLAG, __func__, ret);
...
struct sipa_cal_opt tuning_if_opt = { .init = sipa_afe_init,
				      .exit = sipa_afe_exit,
				      .open = sipa_afe_open,
...
static long sipa_tuning_cmd_unlocked_ioctl(struct file *fp, unsigned int cmd,
					   unsigned long arg)
{
	...
	case SIPA_TUNING_CTRL_WR_UP: {
		...
		if (copy_from_user(priv->cmdup.data, (void __user *)arg,
				   sizeof(dev_comm_data_t))) {
		...
		if (cmd_up->opt == OPT_SET_CAL_VAL) {
			...
			memcpy(cmd_down->payload, cmd_up->payload,
			       cmd_up->payload_size);
	...
	case SIPA_TUNING_CTRL_RD_DOWN: {
		ret = wait_event_interruptible(priv->cmddown.wq,
					       priv->cmddown.flag);
		if (ret) {
		...
		if (copy_to_user((void __user *)arg, priv->cmddown.data,
				 priv->cmddown.len)) {
			return -EFAULT;
		}
		priv->cmddown.flag = false;
		pr_info("[ info][%s] %s: read cmd, len:%d \n", LOG_FLAG,
			__func__, priv->cmddown.len);
	} break;
	default:
...
static long sipa_tuning_tool_unlocked_ioctl(struct file *fp, unsigned int cmd,
					    unsigned long arg)
{
	...
	switch (cmd) {
	case SIPA_TUNING_CTRL_WR_UP: {
		if (copy_from_user(priv->toolup.data, (void __user *)arg,
				   sizeof(dev_comm_data_t))) {
			pr_err("[  err][%s] %s: copy from user failed\n",
			       LOG_FLAG, __func__);
			return -EFAULT;
		...
		if (tool_up->opt == OPT_SET_CAL_VAL) {
			...
			memcpy(tool_down->payload, tool_up->payload,
			       tool_up->payload_size);
	...
	case SIPA_TUNING_CTRL_RD_DOWN: {
		ret = wait_event_interruptible(priv->tooldown.wq,
					       priv->tooldown.flag);
		if (ret) {
			pr_err("[  err][%s] %s: wait_event failed\n", LOG_FLAG,
			       __func__);
		...
		if (copy_to_user((void __user *)arg, priv->tooldown.data,
				 priv->tooldown.len)) {
