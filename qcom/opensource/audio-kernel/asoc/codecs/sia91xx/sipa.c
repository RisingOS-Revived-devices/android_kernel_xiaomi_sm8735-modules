...
static ssize_t sipa_cmd_show(struct device *cd, struct device_attribute *attr,
			     char *buf);
static ssize_t sipa_cmd_store(struct device *cd, struct device_attribute *attr,
...
static const char *support_chip_type_name_table[] = {
	[CHIP_TYPE_SIA8101] = "sia8101",   [CHIP_TYPE_SIA8109] = "sia8109",
	[CHIP_TYPE_SIA8152] = "sia8152",   [CHIP_TYPE_SIA8152S] = "sia8152s",
	[CHIP_TYPE_SIA8100X] = "sia8100x", [CHIP_TYPE_SIA8001] = "sia8001",
	[CHIP_TYPE_SIA8102] = "sia8102",   [CHIP_TYPE_SIA8159] = "sia8159",
	[CHIP_TYPE_SIA8159A] = "sia8159a", [CHIP_TYPE_SIA81X9] = "sia81x9",
	[CHIP_TYPE_SIA8152X] = "sia8152x", [CHIP_TYPE_SIA9195] = "sia9195",
	[CHIP_TYPE_SIA9196] = "sia9196",   [CHIP_TYPE_SIA9175] = "sia9175",
	[CHIP_TYPE_SIA9177] = "sia9177",   [CHIP_TYPE_SIA9255] = "sia9255",
	[CHIP_TYPE_SIA917X] = "sia917x",   [CHIP_TYPE_SIA8150] = "sia8150",
	[CHIP_TYPE_SIA815T] = "sia815T",   [CHIP_TYPE_SIA9187] = "sia9187",
	[CHIP_TYPE_SIA5118] = "sia5118",   [CHIP_TYPE_SIA9197X] = "sia9197x",
	[CHIP_TYPE_SIA8132] = "sia8132",   [CHIP_TYPE_SIA5118S] = "sia5118s",
	[CHIP_TYPE_SIA5118X] = "sia5118x", [CHIP_TYPE_SIA9306] = "sia9306",
};
...
{
	char sia91_rdonly[] = { 0x00, 0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07 };
	int reg[8] = { 0 };
	int ret = 0;
	int i = 0;
	...
	pr_info("%s: ch:%d i2caddr:0x%x 0x00=0x%x 0x01=0x%x 0x02=0x%x 0x03=0x%x 0x04=0x%x 0x05=0x%x 0x06=0x%x 0x07=0x%x\n",
		__func__, si_pa->channel_num, si_pa->client->addr, reg[0],
...
static char sipa_dump_addr[] = {
	...
	0x3d, 0x3e, 0x3f, 0x40, 0x41, 0x50, 0x51, 0x52, 0x53, 0x54, 0x55,
	0x56, 0x57, 0x59, 0x5a, 0x6E, 0x6F, 0x70, 0x71, 0x72, 0x77
};
...
static int sipa_91xx_reg_dump(struct sipa_dev_s *si_pa, char *buff)
{
	...
	for (i = 0; i < arrSize; i++) {
		...
		if (ret == 0) {
			off += snprintf(buff + off, 33,
					"reg addr = %02x, value = 0x%x \n",
					regAddr[i], reg);
			reg = 0;
		} else {
			off += snprintf(buff + off, 40,
					"reg addr = %02x, value = XXXXXX \n",
					regAddr[i]);
		}
	}
 ...
 * si_pa GPIO option
 ********************************************************************/
static __inline void gpio_flipping(int pin, s64 *intervel_ns)
{
...
static __inline int gpio_produce_one_pulse(int pin, int polarity,
					   unsigned int width_us,
					   s64 *real_duty_ns, s64 *real_idle_ns)
{
...
static __inline int __gpio_produce_one_pulse_cycle(int pin, int polarity,
						   unsigned int duty_time_us,
						   unsigned int idle_time_us,
						   s64 *real_duty_ns,
						   s64 *real_idle_ns)
{
	int ret = gpio_produce_one_pulse(pin, polarity, duty_time_us,
...
static __inline int gpio_produce_one_pulse_cycle(int pin, int polarity,
						 unsigned int delay_us,
						 s64 *real_duty_ns,
						 s64 *real_idle_ns)
{
	return __gpio_produce_one_pulse_cycle(pin, polarity, delay_us, delay_us,
					      real_duty_ns, real_idle_ns);
}

static __inline int gpio_produce_pulse_cycles(int pin, int polarity,
					      unsigned int delay_us,
					      unsigned int cycles,
					      s64 *max_real_delay_ns)
{
	...
	while (cycles) {
		...
		ret = gpio_produce_one_pulse_cycle(
			pin, polarity, delay_us, &real_duty_ns, &real_idle_ns);
		if (0 != ret)
			break;

		/* monitor pulse with overtime */
		if (false == is_fst_pulse) {
			max_pulse_time = max_pulse_time > real_idle_ns ?
						 max_pulse_time :
						 real_idle_ns;
		}
		max_pulse_time = max_pulse_time > real_duty_ns ?
					 max_pulse_time :
					 real_duty_ns;
		is_fst_pulse = false;
	}
 ...
 ********************************************************************/
#ifdef OWI_SUPPORT_WRITE_DATA
static void sia81xx_owi_calc_bit_duty_ns(s64 duty_ns, s64 idle_ns,
					 s64 *duty_time_ns, s64 *idle_time_ns,
					 const char bit)
{
	if (0 == !!(bit & (char)0x01)) {
...
static void sia81xx_owi_calc_bit_duty_us(u32 duty_us, u32 idle_us,
					 u32 *duty_time_us, u32 *idle_time_us,
					 const char bit)
{
	s64 duty_time_ns, idle_time_ns;

	sia81xx_owi_calc_bit_duty_ns((s64)duty_us * NSEC_PER_USEC,
				     (s64)idle_us * NSEC_PER_USEC,
...
static int sia81xx_owi_write_one_bit(struct sipa_dev_s *si_pa, const char bit,
				     s64 *real_duty_ns, s64 *real_idle_ns)
{
	int ret = 0;
	...
	sia81xx_owi_calc_bit_duty_us(si_pa->owi_delay_us,
				     MIN_OWI_PULSE_GAP_TIME_US, &duty_time_us,
				     &idle_time_us, bit);

	ret = __gpio_produce_one_pulse_cycle(si_pa->owi_pin,
					     si_pa->owi_polarity, duty_time_us,
					     idle_time_us, real_duty_ns,
					     real_idle_ns);

	return ret;
}

static int sia81xx_owi_start_write_byte(struct sipa_dev_s *si_pa)
{
...
static void sia81xx_owi_end_write_byte(struct sipa_dev_s *si_pa,
				       s64 *real_idle_ns)
{
	gpio_flipping(si_pa->owi_pin, real_idle_ns);
}

static s64 sia81xx_owi_calc_deviation_ns(struct sipa_dev_s *dev,
					 s64 duty_time_ns, s64 idle_time_ns,
					 const char bit)
{
	s64 deviation = 0;

	dev->err_info.owi_max_gap = dev->err_info.owi_max_gap > duty_time_ns ?
					    dev->err_info.owi_max_gap :
					    duty_time_ns;

	dev->err_info.owi_max_gap = dev->err_info.owi_max_gap > idle_time_ns ?
					    dev->err_info.owi_max_gap :
					    idle_time_ns;

	sia81xx_owi_calc_bit_duty_ns(duty_time_ns, idle_time_ns, &duty_time_ns,
				     &idle_time_ns, bit);
...
static int sia81xx_owi_write_one_byte(struct sipa_dev_s *si_pa, const char data)
{
	int ret = 0, i = 0;
	...
	for (i = 7; i >= 0; i--) {
		...
		if (7 > i) {
			cur_deviation_ns = sia81xx_owi_calc_deviation_ns(
				si_pa, last_real_duty_ns, real_idle_ns,
				last_bit);
	...
	if (0 == cur_deviation_ns) {
		...
		cur_deviation_ns = sia81xx_owi_calc_deviation_ns(
			si_pa, last_real_duty_ns, real_idle_ns, last_bit);
		max_deviation_ns = max_deviation_ns > cur_deviation_ns ?
					   max_deviation_ns :
					   cur_deviation_ns;
	}
...
static int sipa_owi_write_data(struct sipa_dev_s *si_pa, unsigned int len,
			       const char *buf)
{
...
static void sia81xx_set_owi_polarity(struct sipa_dev_s *si_pa)
{
	gpio_set_value(si_pa->owi_pin, si_pa->owi_polarity);
}

static int __sipa_owi_write_mode(struct sipa_dev_s *si_pa, unsigned int cycles)
{
	int ret = 0;
	...
	max_flipping_ns = max_flipping_ns > last_flipping_ns ? max_flipping_ns :
							       last_flipping_ns;

	/* record history max flipping time */
	si_pa->err_info.owi_max_gap = si_pa->err_info.owi_max_gap >
						      max_flipping_ns ?
...
static int sipa_owi_write_mode(struct sipa_dev_s *si_pa, unsigned int mode)
{
...
static int sipa_owi_init(sipa_dev_t *si_pa, unsigned int *cfg_mode)
{
...
static bool sipa_is_chip_en(sipa_dev_t *si_pa)
{
	if (0 == si_pa->disable_pin) {
		if (IS_DIGITAL_PA_TYPE(si_pa->chip_type)) {
			...
			if ((SIA81XX_ENABLE_LEVEL ==
			     gpio_get_value(si_pa->rst_pin)) &&
...
int sipa_reg_init(struct sipa_dev_s *si_pa)
{
	...
	if (CHIP_TYPE_SIA8101 == si_pa->chip_type && 0 != si_pa->channel_num) {
		sipa_regmap_defaults(g_default_sia_dev->regmap,
				     si_pa->chip_type, si_pa->scene,
				     si_pa->channel_num);
	} else {
		sipa_regmap_defaults(si_pa->regmap, si_pa->chip_type,
				     si_pa->scene, si_pa->channel_num);
	}

	usleep_range(100, 200);
	if (0 != sipa_regmap_check_chip_id(si_pa->regmap, si_pa->channel_num,
					   si_pa->chip_type)) {
...
static int sipa_resume(struct sipa_dev_s *si_pa)
{
	...
	pr_debug(
		"[debug][%s] %s: si_pa->chip_type = %d, channel:%d, running \n",
		...
		if (0 == si_pa->disable_pin) {
			if (CHIP_TYPE_SIA8101 == si_pa->chip_type &&
				...
				if (likely(NULL != g_default_sia_dev)) {
					default_sia_is_open = gpio_get_value(
						g_default_sia_dev->rst_pin);
			...
			if (IS_SUPPORT_OWI_TYPE(si_pa->chip_type)) {
				...
				if (0 !=
				    sipa_owi_write_mode(
					    si_pa,
					    si_pa->owi_cur_mode[si_pa->scene]))
					goto err_sipa_resume;
			}

			if (IS_DIGITAL_PA_TYPE(si_pa->chip_type) &&
			    !IS_DIGITAL_PA_PULL_RST_TYPE(si_pa->chip_type)) {
				gpio_set_value(si_pa->rst_pin,
					       SIA81XX_DISABLE_LEVEL);
			} else {
				gpio_set_value(si_pa->rst_pin,
					       SIA81XX_ENABLE_LEVEL);
			}
			usleep_range(
				1000,
				1200); /* wait chip power up, the time must be > 1ms */
			...
			pr_debug(
				"[debug][%s] %s: channel = %d, rst_pin_val = %d, owi_mode = %d \n",
				LOG_FLAG, __func__, si_pa->channel_num,
				rst_pin_val, si_pa->owi_cur_mode[si_pa->scene]);
		}
...
static int sipa_suspend(struct sipa_dev_s *si_pa)
{
	...
	if (is_chip_type_supported(si_pa->chip_type) &&
		...
		if (!sipa_regmap_set_chip_off(si_pa) &&
		    IS_DIGITAL_PA_TYPE(si_pa->chip_type)) {
			if (IS_DIGITAL_PA_PULL_RST_TYPE(si_pa->chip_type)) {
				gpio_set_value(si_pa->rst_pin,
					       SIA91XX_LOW_LEVEL);
			} else {
		...
		if (!IS_DIGITAL_PA_TYPE(si_pa->chip_type)) {
			if (0 == si_pa->disable_pin) {
				...
				usleep_range(
					5000,
					6000); /* wait chip power off, the time must be > 5ms */
			}
		}
...
static int sipa_reboot(struct sipa_dev_s *si_pa)
{
	int ret = 0;
...
static int sipa_set_mode(struct sipa_dev_s *si_pa, unsigned int mode)
{
	int ret = 0;
	if (!IS_SUPPORT_OWI_TYPE(si_pa->chip_type)) {
			     ...
			     char *buf)
{
	...
	if (IS_SUPPORT_OWI_TYPE(si_pa->chip_type)) {
		...
		snprintf(buf, 1024,
			 ...
			 "owi_pin = %d,\n"
			 "owi_mode = %u, delay = %u,\n",
			 si_pa->channel_num, si_pa->rst_pin, si_pa->owi_pin,
			 si_pa->owi_cur_mode[si_pa->scene],
			 si_pa->owi_delay_us);
		return strlen(buf);
	}
	...
	case CHIP_TYPE_SIA8109:
		sipa_regmap_read(si_pa->regmap, si_pa->chip_type, 0x41, 1,
				 &chip_id);
		break;
	default:
		sipa_regmap_read(si_pa->regmap, si_pa->chip_type, 0x00, 1,
				 &chip_id);
		break;
	}

	memset(vals, 0, sizeof(vals));
	sipa_regmap_read(si_pa->regmap, si_pa->chip_type, 0x00, 0x2f + 1, vals);
	...
	snprintf(
		...
		si_pa->channel_num, chip_id, vals[0], vals[1], vals[2], vals[3],
		vals[4], vals[5], vals[6], vals[7], vals[8], vals[9], vals[10],
		vals[11], vals[12], vals[13], vals[14], vals[15], vals[16],
		vals[17], vals[18], vals[19], vals[20], vals[21], vals[22],
		vals[23], vals[24], vals[25], vals[26], vals[27], vals[28],
		vals[29], vals[30], vals[31], vals[32], vals[33], vals[34],
		vals[35], vals[36], vals[37], vals[38], vals[39], vals[40],
		vals[41], vals[42], vals[43], vals[44], vals[45], vals[46],
		vals[47], si_pa->rst_pin, si_pa->disable_pin,
			      ...
			      const char *buf, size_t len)
{
	...
	case SIPA_CMD_SET_MODE: {
		...
		sipa_set_mode(si_pa, simple_strtoul(strsep(&cur, split_symb),
						    &after, 10));
		break;
	}
	case SIPA_CMD_GET_REG: {
		...
		addr = (unsigned char)simple_strtoul(strsep(&cur, split_symb),
						     &after, 16);
	...
	case SIPA_CMD_SET_REG: {
		...
		val = (unsigned int)simple_strtoul(strsep(&cur, split_symb),
						   &after, 16);
	...
	case SIPA_CMD_SET_OWI_DELAY: {
		...
		temp_us = (unsigned int)simple_strtoul(strsep(&cur, split_symb),
						       &after, 10);
		if (temp_us <
	...
	case SIPA_CMD_GET_RST_PIN: {
		if (0 == si_pa->disable_pin) {
			...
			pr_debug(
				"[debug][%s] %s: get reset pin value, pin num:%u, value:%u \n",
	...
	case SIPA_CMD_SET_RST_PIN: {
		...
		if (0 == si_pa->disable_pin) {
			...
			usleep_range(
				1000,
				2000); /* wait chip power off, the time must be > 1ms */
			  ...
			  struct snd_ctl_elem_value *ucontrol)
{
	...
	pr_debug(
		"[debug][%s] %s: ucontrol = %ld, rst = %d, channel_num = %d \n",
		LOG_FLAG, __func__, ucontrol->value.integer.value[0],
...
static int sipa_audio_scene_get(struct snd_kcontrol *kcontrol,
				struct snd_ctl_elem_value *ucontrol)
{
#if (LINUX_VERSION_CODE > KERNEL_VERSION(4, 16, 28))
	struct snd_soc_component *component =
...
static int sipa_audio_scene_set(struct snd_kcontrol *kcontrol,
				struct snd_ctl_elem_value *ucontrol)
{
#if (LINUX_VERSION_CODE > KERNEL_VERSION(4, 16, 28))
	struct snd_soc_component *component =
...
static int sipa_pvdd_limit_get(struct snd_kcontrol *kcontrol,
			       struct snd_ctl_elem_value *ucontrol)
{
...
static int sipa_pvdd_limit_set(struct snd_kcontrol *kcontrol,
			       struct snd_ctl_elem_value *ucontrol)
{
#if (LINUX_VERSION_CODE > KERNEL_VERSION(4, 16, 28))
	struct snd_soc_component *component =
...
static int sipa_status_get(struct snd_kcontrol *kcontrol,
			   struct snd_ctl_elem_value *ucontrol)
{
#if (LINUX_VERSION_CODE > KERNEL_VERSION(4, 16, 28))
	struct snd_soc_component *component =
...
static int sipa_status_set(struct snd_kcontrol *kcontrol,
			   struct snd_ctl_elem_value *ucontrol)
{
	return 0;
}

static int sipa_chip_type_get(struct snd_kcontrol *kcontrol,
			      struct snd_ctl_elem_value *ucontrol)
{
#if (LINUX_VERSION_CODE > KERNEL_VERSION(4, 16, 28))
	struct snd_soc_component *component =
...
static int sipa_chip_type_set(struct snd_kcontrol *kcontrol,
			      struct snd_ctl_elem_value *ucontrol)
{
	return 0;
}

static int sipa_mute_get(struct snd_kcontrol *kcontrol,
			 struct snd_ctl_elem_value *ucontrol)
{
#if (LINUX_VERSION_CODE > KERNEL_VERSION(4, 16, 28))
	struct snd_soc_component *component =
...
static int sipa_mute_set(struct snd_kcontrol *kcontrol,
			 struct snd_ctl_elem_value *ucontrol)
{
#if (LINUX_VERSION_CODE > KERNEL_VERSION(4, 16, 28))
	struct snd_soc_component *component =
			...
			unsigned int __user *data, unsigned int size)
{
	...
	pr_debug(
		"[debug][%s] %s: channle = %d size:%d, [%d, %d] 0x%x, 0x%x, 0x%x, 0x%x, addr:0x%x = 0x%x\n",
		LOG_FLAG, __func__, si_pa->channel_num, size, tlv[0], tlv[1],
		regval[TLV_HEADER_SIZE], regval[TLV_HEADER_SIZE + 1],
		regval[TLV_HEADER_SIZE + 2], regval[TLV_HEADER_SIZE + 3], addr,
		value);
			...
			const unsigned int __user *data, unsigned int size)
{
	...
	addr = (regval[TLV_HEADER_SIZE] << 8) | regval[TLV_HEADER_SIZE + 1];
	value = (regval[TLV_HEADER_SIZE + 2] << 8) |
		regval[TLV_HEADER_SIZE + 3];
	pr_debug(
		"[debug][%s] %s: channle = %d size:%d, [%d,%d] 0x%x,0x%x,0x%x,0x%x addr:0x%x = 0x%x \n",
		LOG_FLAG, __func__, si_pa->channel_num, size, tlv[0], tlv[1],
		regval[TLV_HEADER_SIZE], regval[TLV_HEADER_SIZE + 1],
...
static const char *const audio_scene[] = { "Playback", "Voice",	  "Voip",
					   "Receiver", "Factory", "FM" };
...
static int sia91xx_spksw_gpio_get(struct snd_kcontrol *kcontrol,
				  struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *component =
		snd_soc_kcontrol_component(kcontrol);
	...
	int current_level = gpio_get_value_cansleep(si_pa->spksw_gpio);
	dev_info(&si_pa->client->dev, "spksw level cached = %d, actual = %d\n",
		 si_pa->spksw_level, current_level);

	if (current_level < 0) {
		// failed to read the actual value, fallback to cached value
		ucontrol->value.enumerated.item[0] = si_pa->spksw_level;
	} else {
...
static int sia91xx_spksw_gpio_put(struct snd_kcontrol *kcontrol,
				  struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *component =
		snd_soc_kcontrol_component(kcontrol);
	...
	dev_info(&si_pa->client->dev, "spksw = %d\n", spksw_gpio);

	gpio_set_value_cansleep(si_pa->spksw_gpio, !!spksw_gpio);
...
static const struct snd_kcontrol_new sipa_controls[] = {
	...
	SOC_ENUM_EXT("Sipa Power", power_enum, sipa_power_get, sipa_power_set),

	SOC_ENUM_EXT("Sipa Audio Scene", audio_scene_enum, sipa_audio_scene_get,
		     sipa_audio_scene_set),

	SOC_SINGLE_EXT("Sipa Pvdd Limit", SND_SOC_NOPM, 0, 5000000, 0,
		       sipa_pvdd_limit_get, sipa_pvdd_limit_set),

	SOC_SINGLE_EXT("Sipa Status", SND_SOC_NOPM, 0, 0xff, 0, sipa_status_get,
		       sipa_status_set),

	SOC_SINGLE_EXT("Sipa Type", SND_SOC_NOPM, 0, 100, 0, sipa_chip_type_get,
		       sipa_chip_type_set),

	SOC_SINGLE_EXT("Sipa Mute", SND_SOC_NOPM, 0, 1, 0, sipa_mute_get,
		       sipa_mute_set),

	SND_SOC_BYTES_TLV("Sipa reg", REG_DATA_LEN, sipa_reg_get, sipa_reg_set),

	SOC_SINGLE_EXT("sia91xx_spk_sw_gpio_switch", SND_SOC_NOPM, 0, 0x3, 0,
		       sia91xx_spksw_gpio_get, sia91xx_spksw_gpio_put),
};
...
static int sipa_spkr_pa_event(struct snd_soc_dapm_widget *w,
...
static const struct snd_soc_dapm_widget sipa_dapm_widgets[] = {
	...
	SND_SOC_DAPM_OUT_DRV_E("SPKR DRV", 0, 0, 0, NULL, 0, sipa_spkr_pa_event,
...
#if (LINUX_VERSION_CODE > KERNEL_VERSION(4, 16, 28))
static int sipa_component_probe(struct snd_soc_component *component)
{
	sipa_dev_t *si_pa = snd_soc_component_get_drvdata(component);
	if (NULL == si_pa) {
		pr_err("[  err][%s] %s: si_pa is NULL\n", LOG_FLAG, __func__);
...
static void sipa_component_remove(struct snd_soc_component *component)
{
	pr_info("[debug][%s] %s: running \n", LOG_FLAG, __func__);
	return;
}
#else
static int sipa_codec_probe(struct snd_soc_codec *codec)
{
	sipa_dev_t *si_pa = snd_soc_codec_get_drvdata(codec);
	if (NULL == si_pa) {
		pr_err("[  err][%s] %s: si_pa is NULL\n", LOG_FLAG, __func__);
...
static int sipa_codec_remove(struct snd_soc_codec *codec)
{
	pr_info("[debug][%s] %s: running \n", LOG_FLAG, __func__);
	return 0;
}
...
static struct snd_soc_component_driver soc_component_dev_sipa = {
	...
	.read = sipa_component_read,
       ...
       LINUX_VERSION_CODE <= KERNEL_VERSION(4, 16, 28))
...
static sipa_dev_t *create_sipa_dev(struct device *dev)
{
	...
	/* default name */
	snprintf(si_pa->name, strlen("sipa-spk.%u"), "sipa-spk.%u", i);

	pr_debug("[debug][%s] %s: add si_pa dev : %s, index = %u \n", LOG_FLAG,
		 __func__, si_pa->name, i);
	return si_pa;
}
...
static void put_sipa_dev(sipa_dev_t *si_pa)
{
	...
	pr_debug("[debug][%s] %s: put dev name : %s, pdev = %p, client = %p \n",
		 LOG_FLAG, __func__, si_pa->name, si_pa->pdev, si_pa->client);
...
/* CHIP_TYPE_SIA81X9 */
static const uint32_t sia81x9_list[] = { CHIP_TYPE_SIA815T, CHIP_TYPE_SIA8159,
					 CHIP_TYPE_SIA8159A,
					 CHIP_TYPE_SIA8109 };
...
static const uint32_t sia8152x_list[] = {
	...
	CHIP_TYPE_SIA8152, CHIP_TYPE_SIA8150, CHIP_TYPE_SIA8132
};
...
static const struct sipa_chip_compat sipa_compat_table[] = {
	{ CHIP_TYPE_SIA81X9, { sia81x9_list, ARRAY_SIZE(sia81x9_list) } },
	{ CHIP_TYPE_SIA8152X, { sia8152x_list, ARRAY_SIZE(sia8152x_list) } },
	{ CHIP_TYPE_SIA917X, { sia917x_list, ARRAY_SIZE(sia917x_list) } },
	{ CHIP_TYPE_SIA5118X, { sia5118x_list, ARRAY_SIZE(sia5118x_list) } }
};
...
{
	if (0 == si_pa->disable_pin &&
	    IS_NEED_PULL_RST_TYPE(si_pa->chip_type)) {
		gpio_set_value(si_pa->rst_pin, value);
...
static void distinguish_chip_type(sipa_dev_t *si_pa)
{
	...
	if (CHIP_TYPE_UNKNOWN == si_pa->chip_type) {
		...
		if (1 == si_pa->en_dyn_id) {
			...
			device_create_file(&si_pa->pdev->dev,
					   &dev_attr_sipa_device);
			...
			if (0 != check_sipa_status(si_pa)) {
				...
				pr_info("[ info][%s] %s:  failed to check_sipa_status\n",
					LOG_FLAG, __func__);
			} else {
				device_create_file(&si_pa->pdev->dev,
						   &dev_attr_sipa_device);
...
void sipa_compatible_chips_adapt(sipa_dev_t *si_pa)
{
	int i = 0, j = 0;
		...
		if (si_pa->chip_type == sipa_compat_table[i].sub_type) {
			...
			for (j = 0; j < sipa_compat_table[i].num; j++) {
				if (NULL != sipa_compat_table[i].chips &&
				    0 == sipa_regmap_check_chip_id(
						 si_pa->regmap,
						 si_pa->channel_num,
						 sipa_compat_table[i].chips[j])) {
					si_pa->chip_type =
						sipa_compat_table[i].chips[j];
					break;
				}
...
int sipa_pending_actions(sipa_dev_t *si_pa)
{
	...
	/* copy settings */
	if (NULL == sipa_param_read_chip_cfg(si_pa->channel_num,
					     si_pa->chip_type, &chip_cfg)) {
		pr_err("[  err][%s] %s: read_chip_cfg return NULL \n", LOG_FLAG,
		       __func__);
		return -EFAULT;
	}

	if (0 != sipa_param_read_extra_cfg(si_pa->channel_num, &extra_cfg))
		return -EFAULT;

	si_pa->dyn_ud_vdd_port = extra_cfg.dyn_ud_vdd_port;
	g_dyn_ud_vdd_port =
		extra_cfg.dyn_ud_vdd_port; //qcom need afe port 0x1004
	...
	pr_info("[ info][%s] %s: sipa %d driver init done \n", LOG_FLAG,
		__func__, si_pa->channel_num);

	return 0;
}

#if (LINUX_VERSION_CODE < KERNEL_VERSION(6, 6, 0))
int sipa_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
#else
...
#endif
{
	...
	pr_info("[ info][%s] %s: i2c addr = 0x%02x \n", LOG_FLAG, __func__,
		client->addr);
	if (!check_smartpa_type("sia91xx")) {
		pr_warn("[ warn][%s] %s: smartpa type %d already probed, abort",
			LOG_FLAG, __func__, get_smartpa_type());
		return -ENODEV;
	}
	...
	sipa_of_node = of_parse_phandle(client->dev.of_node, "si,sipa-dev", 0);
	if (!sipa_of_node) {
	...
	ret = of_property_read_s32(client->dev.of_node, "si,sipa-addr_offset",
				   &addr_offset);
	if (0 != ret) {
	...
	si_pa->regmap =
		sipa_regmap_init(client, si_pa->channel_num,
				 get_one_available_chip_type(si_pa->chip_type));
	if (IS_ERR(si_pa->regmap)) {
		pr_err("[  err][%s] %s: regmap_init_i2c error [TF-NOSOUND]!!! \n",
		       LOG_FLAG, __func__);
	...
	if (si_pa->chip_type == CHIP_TYPE_SIA8152X) {
		...
		if (ret < 0) {
			...
			client->addr -= 0x30;
			si_pa->regmap = sipa_regmap_init(
				client, si_pa->channel_num,
				get_one_available_chip_type(si_pa->chip_type));
			if (IS_ERR(si_pa->regmap)) {
	...
	if (gpio_is_valid(si_pa->rst_pin)) {
		ret = devm_gpio_request_one(&client->dev, si_pa->rst_pin,
					    GPIOF_OUT_INIT_LOW, "SIA91XX_RST");
		if (ret) {
			pr_err("[ err][%s] %s: rst gpio request failed!\n",
			       LOG_FLAG, __func__);
			if (ret == -EBUSY)
				return -EPROBE_DEFER;
		}
	...
	if (IS_DIGITAL_PA_TYPE(si_pa->chip_type)) {
		...
		set_smartpa_type("sia91xx", sizeof("sia91xx"));

		if (gpio_is_valid(si_pa->spksw_gpio)) {
			ret = devm_gpio_request_one(&client->dev,
						    si_pa->spksw_gpio,
						    GPIOF_OUT_INIT_LOW,
						    "sia91xx_spksw");
			if (ret) {
				pr_err("[  err][%s] %s: spksw gpio request failed!\n",
				       LOG_FLAG, __func__);
				return ret;
			}
		}

		dai = devm_kzalloc(&client->dev, sizeof(sia91xx_dai),
				   GFP_KERNEL);
		if (NULL == dai)
			return -ENOMEM;
		...
		sia91xx_append_i2c_address(&client->dev, client, NULL, 0, dai,
					   ARRAY_SIZE(sia91xx_dai));

#if (LINUX_VERSION_CODE > KERNEL_VERSION(4, 16, 28))
		ret = devm_snd_soc_register_component(&client->dev,
						      &soc_component_dev_sipa,
						      dai,
						      ARRAY_SIZE(sia91xx_dai));
#else
		ret = snd_soc_register_codec(&client->dev, &soc_codec_dev_sipa,
					     dai, ARRAY_SIZE(sia91xx_dai));
#endif
		if (ret < 0) {
			dev_err(&client->dev,
				"Failed to register sia91xx: %d [TF-NOSOUND]\n",
				ret);
			return ret;
		}
		pr_info("[info][%s] snd_soc_register_codec ret = %d !\n",
			__func__, ret);
		set_smartpa_codec(&client->dev, dai[0].name);
	}

	if (CHIP_TYPE_UNKNOWN != si_pa->chip_type &&
	    !IS_SUPPORT_OWI_TYPE(si_pa->chip_type)) {
	...
	pr_info("[ info][%s] %s: finish i2c addr = 0x%02x, name:%s, channel:%d\n",
		LOG_FLAG, __func__, client->addr, si_pa->name,
		si_pa->channel_num);
...
static const struct i2c_device_id si_sipa_i2c_id[] = { { SIPA_I2C_NAME, 0 },
						       {} };
...
static int sipa_pm_suspend(struct device *dev)
{
#if 0
	sipa_dev_t *si_pa = NULL;
...
static const struct dev_pm_ops si_sipa_pm_ops = { SET_SYSTEM_SLEEP_PM_OPS(
	sipa_pm_suspend, sipa_pm_resume) };

static int sipa_property_init(struct device_node *sipa_of_node,
			      sipa_dev_t *si_pa)
{
	int ret = 0;
	...
	/* get chip type name */
	ret = of_property_read_string_index(sipa_of_node, "si,si_pa_type", 0,
					    &chip_type_name);
	if (0 != ret) {
	...
	ret = of_property_read_u32(sipa_of_node, "si,si_pa_disable_pin",
				   &disable_pin);
	if (0 != ret) {
	...
	ret = of_property_read_u32(sipa_of_node, "en_compatible_type_func",
				   &en_compatible_type);
	if (0 != ret) {
	...
	pr_debug(
		"[debug][%s] %s: channel_num = %d, en_compatible_type = %d, scene = %d, "
		"disable_pin = %d, en_dyn_id = %d\n",
		LOG_FLAG, __func__, si_pa->channel_num,
		si_pa->en_compatible_type, si_pa->scene, si_pa->disable_pin,
		si_pa->en_dyn_id);
	...
	if (0 == si_pa->disable_pin) {
		...
		if (IS_DIGITAL_PA_TYPE(si_pa->chip_type)) {
			if (si_pa->chip_type == CHIP_TYPE_SIA5118) {
				boost_pin = of_get_named_gpio(
					sipa_of_node, "si,si_pa_boost", 0);
				if (boost_pin < 0) {
					pr_err("[  err][%s] %s: boost_pin < 0 !!! \n",
					       LOG_FLAG, __func__);
					return -ENODEV;
				}
				si_pa->boost_pin = boost_pin;
			}
			ret = of_property_read_u32(sipa_of_node, "en_irq_func",
						   &en_irq_pin);
			if ((0 != ret) || (1 != en_irq_pin)) {
				...
				irq_pin = of_get_named_gpio(sipa_of_node,
							    "si,si_pa_irq", 0);
				if (irq_pin < 0) {
		...
		if (IS_DIGITAL_PA_TYPE(si_pa->chip_type)) {
			spksw_gpio = of_get_named_gpio(sipa_of_node,
						       "spksw-gpio", 0);
			if (spksw_gpio < 0) {
				pr_err("[  err][%s] %s: spksw_gpio < 0 !!! \n",
				       LOG_FLAG, __func__);
			}
			si_pa->spksw_gpio = spksw_gpio;
			pr_info("[  info][%s] %s: spksw_gpio = %d \n", LOG_FLAG,
				__func__, si_pa->spksw_gpio);
		}

		/* get owi pin's sn */
		if (IS_SUPPORT_OWI_TYPE(si_pa->chip_type)) {
			owi_pin = of_get_named_gpio(sipa_of_node,
						    "si,si_pa_owi", 0);
			if (owi_pin < 0) {
		...
		if (1 == si_pa->en_dyn_id) {
			/* get id pin's sn */
			id_pin = of_get_named_gpio(sipa_of_node, "si,si_pa_id",
						   0);
			if (id_pin < 0) {
...
static int sipa_pinctrl_select(struct platform_device *pdev, sipa_dev_t *si_pa)
{
	...
	if (0 == si_pa->disable_pin) {
		...
		pinctrl_state =
			pinctrl_lookup_state(si_pa_pinctrl, "si_pa_gpio");
		...
		if (IS_DIGITAL_PA_TYPE(si_pa->chip_type)) {
			if (si_pa->chip_type == CHIP_TYPE_SIA5118 &&
			    si_pa->boost_pin > 0) {
				pinctrl_state = pinctrl_lookup_state(
					si_pa_pinctrl, "si_pa_boost");
				if (NULL == pinctrl_state) {
					pr_err("[  err][%s] %s: si_pa_boost NULL == pinctrl_state !!! \n",
					       LOG_FLAG, __func__);
					ret = -ENODEV;
					goto err;
				}
				...
				/* set this pinctrl state, make this pin works in the gpio mode */
				ret = pinctrl_select_state(si_pa_pinctrl,
							   pinctrl_state);
				if (0 != ret) {
					pr_err("[  err][%s] %s: error si_pa_boost pinctrl_select_state return %d \n",
					       LOG_FLAG, __func__, ret);
					ret = -ENODEV;
					goto err;
				}

				if (gpio_is_valid(si_pa->boost_pin)) {
		...
		if (1 == si_pa->en_dyn_id) {
			pinctrl_state =
				pinctrl_lookup_state(si_pa_pinctrl, "si_pa_id");
			if (NULL == pinctrl_state) {
				pr_err("[  err][%s] %s: NULL == pinctrl_state !!! \n",
				       LOG_FLAG, __func__);
				ret = -ENODEV;
		...
		if (IS_DIGITAL_PA_TYPE(si_pa->chip_type)) {
			if (gpio_is_valid(si_pa->irq_pin)) {
				ret = devm_gpio_request_one(&pdev->dev,
							    ...
							    "SIA91XX_INT");
				if (ret) {
					pr_err("%s: fail to request irq gpio %d",
					       __func__, ret);
					return ret;
				}
			}
...
static int sipa_probe(struct platform_device *pdev)
{
	...
	if (!IS_DIGITAL_PA_TYPE(si_pa->chip_type)) {
		...
		ret = devm_snd_soc_register_component(
			&pdev->dev, &soc_component_dev_sipa, NULL, 0);
#else
		ret = snd_soc_register_codec(&pdev->dev, &soc_codec_dev_sipa,
					     NULL, 0);
#endif
		if (0 != ret) {
