...
const struct sia91xx_irq_desc irq_range[] = {
	{ 0x1, "PORI" },  { 0x4, "NOCLK" }, { 0x8, "OTPI" },
	{ 0x20, "UVPI" }, { 0x40, "OCPI" }, { 0x80, "TDMERR" },
};
...
void sia91xx_register_interrupt(sipa_dev_t *si_pa)
{
	...
	if (gpio_is_valid(si_pa->irq_pin)) {
		...
		if (si_pa->channel_num == SIPA_CHANNEL_0) {
			ret = request_irq(gpio_to_irq(si_pa->irq_pin),
					  sia91xx_irq, irq_flags, "sia91xx_L",
					  si_pa);
		} else {
			ret = request_irq(gpio_to_irq(si_pa->irq_pin),
					  sia91xx_irq, irq_flags, "sia91xx_R",
					  si_pa);
		}
	} else {
...
static int sia91xx_ext_reset(sipa_dev_t *si_pa)
{
	if (si_pa && gpio_is_valid(si_pa->rst_pin)) {
		if (IS_DIGITAL_PA_PULL_RST_TYPE(si_pa->chip_type)) {
			gpio_set_value_cansleep(si_pa->rst_pin,
						SIA91XX_HIGH_LEVEL);
...
int sia91xx_append_i2c_address(struct device *dev, struct i2c_client *i2c,
		      ...
		      struct snd_pcm_hw_params *params, struct snd_soc_dai *dai)
{
	...
	pr_info("[ info][%s] %s: dai:%s %s i2s rate: %u !!! \n", LOG_FLAG,
		__func__, dai->name, substream->name, rate);
	...
	pr_debug(
		"[debug][%s] %s: stream = %d, requested rate = %d, sample size = %d,"
...
int sia91xx_startup(struct snd_pcm_substream *substream,
		    struct snd_soc_dai *dai)
{
	...
	pr_debug(
		"[debug][%s] %s: dai:%s, substream:%s, startup, stream = %d \n",
		LOG_FLAG, __func__, dai->name, substream->name,
		substream->stream);
	...
	if (si_pa->power_mode == true) {
		pr_info("[debug][%s] %s: si_pa->power_mode is true, direct return!\n",
			LOG_FLAG, __func__);
		return 0;
	}
...
int sia91xx_mute(struct snd_soc_dai *dai, int mute, int stream)
{
	...
	pr_debug("[debug][%s] %s: dai:%s,%s mute = %d stream = %d\n", LOG_FLAG,
		 __func__, dai->name, dai->driver->name, mute, stream);
	...
	if (stream == SNDRV_PCM_STREAM_PLAYBACK) {
		if (mute) {
			if (si_pa->power_mode == false) {
				pr_info("[debug][%s] %s: si_pa->power_mode is false, direct return!\n",
					LOG_FLAG, __func__);
				return 0;
			}
			...
			if (sia91xx_smartpa_soft_mute(si_pa)) {
				if (IS_DIGITAL_PA_PULL_RST_TYPE(
					    si_pa->chip_type)) {
		...
		if (si_pa->chip_type == CHIP_TYPE_SIA5118) {
			...
			pr_debug(
				"[debug][%s] %s: channel = %d, boost_pin_val = %d \n",
...
int sia91xx_component_probe(struct snd_soc_component *component)
{
	...
	sia91xx_append_i2c_address(&si_pa->client->dev, si_pa->client, widgets,
