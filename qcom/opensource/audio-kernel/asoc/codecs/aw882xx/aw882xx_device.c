...
/*********************************algo auth*************************************/
static int aw_dev_get_encrypted_value(struct aw_device *aw_dev, unsigned int in,
				      unsigned int *out)
{
	int ret = 0;
...
int aw_dev_algo_auth_dsp_mode(struct aw_device *aw_dev,
			      struct algo_auth_data *algo_data)
{
	...
	if (algo_data->auth_mode == AW_ALGO_AUTH_MODE_REG_CRC) {
		ret = aw_dev_get_encrypted_value(aw_dev, algo_data->random,
						 &encrypted_out);
		if (ret < 0)
...
void aw_dev_algo_authentication(struct aw_device *aw_dev)
{
	...
	ret = aw882xx_dsp_read_algo_auth_data(aw_dev, (char *)&algo_data,
					      sizeof(struct algo_auth_data));
	if (ret < 0)
		goto exit;
	...
	ret = aw882xx_dsp_write_algo_auth_data(aw_dev, (char *)&algo_data,
					       sizeof(struct algo_auth_data));
	if (ret < 0)
		goto exit;
	...
	aw_dev_dbg(aw_dev->dev,
		   "mode=%d,reg_crc=0x%x,random=0x%x,id=0x%x,res=%d",
...
static int aw_algo_auth_misc_ops(struct aw_device *aw_dev, unsigned int cmd,
				 unsigned long arg)
{
	int ret = 0;
...
#ifdef CONFIG_COMPAT
static long aw_algo_auth_misc_compat_ioctl(struct file *file, unsigned int cmd,
					   unsigned long arg)
{
	int ret = 0;
	struct aw_device *aw_dev = NULL;
...
static void aw_dev_reg_dump(struct aw_device *aw_dev)
{
	...
	for (i = 0; i < reg_num; i++) {
		...
		aw_dev_info(aw_dev->dev, "read: reg = 0x%02x, val = 0x%04x", i,
			    reg_val);
	}
}
...
static int aw_dev_get_icalk(struct aw_device *aw_dev, int16_t *icalk)
{
	...
	if (desc->icalkl_reg == AW_REG_NONE) {
		ret = aw_dev->ops.aw_i2c_read(aw_dev->i2c, desc->icalk_reg,
					      &reg_val);
		...
		ret = aw_dev->ops.aw_i2c_read(aw_dev->i2c, desc->icalk_reg,
					      &reg_val);
		reg_icalk = (uint16_t)reg_val & (~desc->icalk_reg_mask);
		ret = aw_dev->ops.aw_i2c_read(aw_dev->i2c, desc->icalkl_reg,
					      &reg_val);
...
static int aw_dev_get_vcalk(struct aw_device *aw_dev, int16_t *vcalk)
{
	...
	if (desc->vcalkl_reg == AW_REG_NONE) {
		ret = aw_dev->ops.aw_i2c_read(aw_dev->i2c, desc->vcalk_reg,
					      &reg_val);
		...
		ret = aw_dev->ops.aw_i2c_read(aw_dev->i2c, desc->vcalk_reg,
					      &reg_val);
		reg_vcalk = (uint16_t)reg_val & (~desc->vcalk_reg_mask);
		ret = aw_dev->ops.aw_i2c_read(aw_dev->i2c, desc->vcalkl_reg,
					      &reg_val);
...
static unsigned int aw_dev_reg_value_check(struct aw_device *aw_dev,
					   ...
					   unsigned int reg_val)
{
	...
	if (profctrl_desc->reg != AW_REG_NONE) {
		if (reg_addr == profctrl_desc->reg) {
			profctrl_desc->cfg_mode = reg_val &
						  (~profctrl_desc->mask);
		}

		if (reg_addr == bstctrl_desc->reg) {
			bstctrl_desc->cfg_type = reg_val &
						 (~bstctrl_desc->mask);
		}
	}
	...
	/*get efuse_check*/
	if ((aw_dev->efcheck_desc.reg != AW_REG_NONE) &&
	    (reg_addr == aw_dev->efcheck_desc.reg)) {
	...
	if (reg_addr == aw_dev->volume_desc.reg) {
		if (aw_dev->ops.aw_reg_val_to_db) {
			aw_dev->volume_desc.init_volume =
				aw_dev->ops.aw_reg_val_to_db(
					(reg_val &
					 (~aw_dev->volume_desc.mask)) >>
					aw_dev->volume_desc.shift);
		} else {
			aw_dev_dbg(
				aw_dev->dev,
				"aw_reg_val_to_db is NULL, can't get init_volume");
		}
	}
...
static void aw_dev_switch(struct aw_device *aw_dev, struct aw_switch_desc *btn,
			  bool enable)
{
	aw_dev_dbg(aw_dev->dev, "enter %s", btn->name);

	if (enable) {
		aw_dev->ops.aw_i2c_write_bits(aw_dev->i2c, btn->reg, btn->mask,
					      btn->enable);
	} else {
		aw_dev->ops.aw_i2c_write_bits(aw_dev->i2c, btn->reg, btn->mask,
					      btn->disable);
...
static int aw_dev_sysst_check(struct aw_device *aw_dev)
{
	...
	if (noise_gate_desc->reg != AW_REG_NONE) {
		aw_dev->ops.aw_i2c_read(aw_dev->i2c, noise_gate_desc->reg,
					&reg_val);
...
static void aw_dev_vol_offset_update(struct aw_device *aw_dev)
{
	...
	if (aw_dev->ops.aw_get_voltage_offset)
		...
		aw_dev_info(aw_dev->dev,
			    "unsupport, ignore voltage offset update");
		return;
	}
...
void aw_dev_i2s_enable(struct aw_device *aw_dev, bool flag)
{
	...
	if (flag) {
		...
		aw_dev->ops.aw_i2c_write_bits(aw_dev->i2c, txen_desc->reg,
					      txen_desc->mask,
					      txen_desc->disable);
	}
}
...
int aw882xx_dev_set_volume(struct aw_device *aw_dev, unsigned int set_vol)
{
	...
	if (aw_dev->ops.aw_db_val_to_reg)
		real_value = aw_dev->ops.aw_db_val_to_reg(
			AW_GET_MIN_VALUE(hw_vol, vol_desc->mute_volume));
	else
	...
	/* mask volume */
	real_value = (real_value << vol_desc->shift) |
		     (reg_value & vol_desc->mask);
...
int aw882xx_dev_get_volume(struct aw_device *aw_dev, unsigned int *get_vol)
{
	...
	if (aw_dev->ops.aw_reg_val_to_db)
		real_value = aw_dev->ops.aw_reg_val_to_db(
			AW_GET_MIN_VALUE(real_value, vol_desc->mute_volume));
	else
		real_value =
			AW_GET_MIN_VALUE(real_value, vol_desc->mute_volume);

	aw_dev_dbg(aw_dev->dev, "reg_value %d ,real_value 0x%x", reg_value,
		   real_value);
...
int aw882xx_dev_algo_auth_get(struct aw_device *aw_dev,
			      struct algo_auth_data *algo_data)
{
...
int aw882xx_dev_algo_auth_set(struct aw_device *aw_dev,
			      struct algo_auth_data *algo_data)
{
	int ret = 0;
	unsigned int encrypted_out = 0;
	...
	case AW_ALGO_AUTH_MODE_REG_CRC:
		...
		break;
	default:
		aw_dev_err(aw_dev->dev, "unsupport auth mode[%d]",
			   algo_data->auth_mode);
		ret = -EINVAL;
	}
...
int aw882xx_dev_get_int_status(struct aw_device *aw_dev, uint16_t *int_status)
{
	...
	ret = aw_dev->ops.aw_i2c_read(aw_dev->i2c, aw_dev->int_desc.st_reg,
				      &reg_val);
	if (ret < 0)
...
static int aw_dev_parse_dt(struct aw_device *aw_dev)
{
	...
	aw_dev->pll_check_disable = of_property_read_bool(aw_dev->dev->of_node,
							  "pll-check-disable");
	if (aw_dev->pll_check_disable) {
...
int aw882xx_dev_check_ef_lock(struct aw_device *aw_dev)
{
	...
	for (i = 0; i < ef_desc->count; i++) {
		aw_dev->ops.aw_i2c_read(aw_dev->i2c, ef_desc->sequence[i].reg,
					&reg_val);
