...
static int aw882xx_i2c_writes(struct i2c_client *i2c, unsigned char reg_addr,
...
static int aw882xx_i2c_reads(struct i2c_client *i2c, unsigned char reg_addr,
...
static int aw882xx_i2c_write(struct i2c_client *i2c, unsigned char reg_addr,
			     unsigned int reg_data)
{
	...
	while (cnt < AW_I2C_RETRIES) {
		...
		if (ret < 0) {
			aw_dev_err(&i2c->dev, "i2c_write cnt=%d error=%d", cnt,
				   ret);
		} else {
			if (g_print_dbg)
				aw_dev_info(
...
static int aw882xx_i2c_read(struct i2c_client *i2c, unsigned char reg_addr,
			    unsigned int *reg_data)
{
	int ret = -1;
	unsigned char cnt = 0;
	...
	while (cnt < AW_I2C_RETRIES) {
		...
		if (ret < 0) {
			aw_dev_err(&i2c->dev, "i2c_read cnt=%d error=%d", cnt,
				   ret);
		} else {
	...
	if (aw882xx->fw_status == AW_DEV_FW_OK) {
		...
		for (i = 0; i < AW_START_RETRIES; i++) {
			...
			if (ret) {
				aw_dev_err(
					aw882xx->dev,
					"start failed, cnt:%d[TF-NOSOUND][AW]",
					i);
#if IS_ENABLED(CONFIG_MIEV)
...
static int aw882xx_dynamic_create_controls(struct aw882xx *aw882xx)
{
	...
	snprintf(kctl_name, AW_NAME_BUF_MAX, "aw_dev_%d_offset",
		 aw882xx->aw_pa->channel);

	aw882xx_dev_control[KCTL_TYPE_VOLTAGE_OFFSET].name = kctl_name;
	aw882xx_dev_control[KCTL_TYPE_VOLTAGE_OFFSET].iface =
		SNDRV_CTL_ELEM_IFACE_MIXER;
	aw882xx_dev_control[KCTL_TYPE_VOLTAGE_OFFSET].info =
		aw882xx_voltage_offset_info;
	aw882xx_dev_control[KCTL_TYPE_VOLTAGE_OFFSET].get =
		aw882xx_voltage_offset_get;
	...
	snprintf(kctl_name, AW_NAME_BUF_MAX, "aw_dev_%d_iv_output",
		 aw882xx->aw_pa->channel);

	aw882xx_dev_control[KCTL_TYPE_IV_OUTPUT].name = kctl_name;
	aw882xx_dev_control[KCTL_TYPE_IV_OUTPUT].iface =
		SNDRV_CTL_ELEM_IFACE_MIXER;
...
static void aw882xx_request_firmware(struct work_struct *work)
{
	...
	aw_dev_info(aw882xx->dev, "add dynamic controls done");
...
static void aw882xx_add_codec_controls(struct aw882xx *aw882xx)
{
	...
	aw882xx_dev_control[0].access = SNDRV_CTL_ELEM_ACCESS_TLV_CALLBACK |
...
#if defined(PROBE_FAILED_REGISTER_DUMMY_DAI)
static struct snd_soc_dai_driver aw_dummy_dai = {
	.name = "aw882xx-aif",
	.id = 0,
	.playback = {
		.stream_name = "Speaker_Playback",
...
static struct snd_soc_component_driver aw_dummy_codec;
#endif
...
static int aw882xx_read_chipid(struct aw882xx *aw882xx)
{
	...
	while (cnt < AW_READ_CHIPID_RETRIES) {
		ret = aw882xx_i2c_read(aw882xx->i2c, AW882XX_CHIP_ID_REG,
				       &reg_value);
		if (ret < 0) {
			 ...
			 char *buf)
{
	...
	ret = aw882xx_i2c_reads(aw882xx->i2c, packet->reg_addr,
				...
				size_t count)
{
	...
	if (sscanf(buf, "%d %d", &databuf[0], &databuf[1]) == 2) {
		if (databuf[0])
			aw882xx_dsp_write_vol_offset(aw882xx->aw_pa,
						     databuf[1]);
...
static struct attribute *aw882xx_attributes[] = {
	&dev_attr_reg.attr,	   &dev_attr_rw.attr,
	&dev_attr_awrw.attr,	   &dev_attr_drv_ver.attr,
	&dev_attr_fade_step.attr,  &dev_attr_dbg_prof.attr,
	&dev_attr_dsp_re.attr,	   &dev_attr_phase_sync.attr,
	&dev_attr_print_dbg.attr,  &dev_attr_algo_ver.attr,
	&dev_attr_vol_offset.attr, NULL
};
...
#if defined(PROBE_FAILED_REGISTER_DUMMY_DAI)
	struct snd_soc_dai_driver *dummy_dai_drv = NULL;
#endif
...
#if defined(PROBE_FAILED_REGISTER_DUMMY_DAI)
		aw_dev_info(&i2c->dev, "register dummy codec\n");
		memset(&aw_dummy_codec, 0,
		       sizeof(struct snd_soc_component_driver));
		dummy_dai_drv = &aw_dummy_dai;
		dummy_dai_drv->name =
			devm_kstrdup(aw882xx->dev, "aw882xx-aif", GFP_KERNEL);
		if (!(dummy_dai_drv->name)) {
			aw_dev_err(&i2c->dev, "failed to duplicate string\n");
			return -ENOMEM;
		}
		ret = aw882xx_append_i2c_suffix("%s-%d-%x",
						&dummy_dai_drv->name, aw882xx);
		if (ret < 0) {
			aw_dev_err(&i2c->dev,
				   "aw882xx_append_i2c_suffix failed");
		}
		ret = snd_soc_register_component(aw882xx->dev, &aw_dummy_codec,
						 &aw_dummy_dai, 1);
		if (ret < 0) {
			aw_dev_err(&i2c->dev,
				   "%s: Register dummy codec failed\n",
				   __func__);
		}
		return 0;
#endif
