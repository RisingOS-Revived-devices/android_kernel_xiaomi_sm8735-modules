...
{
	return (((value / AW_6_0P5_VOL_STEP_DB) << 4) +
		(value % AW_6_0P5_VOL_STEP_DB));
}
...
{
	return (((value / AW_6_0P125_VOL_STEP_DB) << 6) +
		(value % AW_6_0P125_VOL_STEP_DB));
}
...
static void aw_pid_2013_efver_check(struct aw_device *aw_dev)
{
	...
	aw_dev->ops.aw_i2c_read(aw_dev->i2c, AW_PID_2013_EFRM1_REG, &reg_val);
...
static int aw_pid_2055_dev_check(struct aw_device *aw_pa)
{
	...
	aw_pa->ops.aw_i2c_write(aw_pa->i2c, AW882XX_SOFT_RESET_REG,
				AW882XX_SOFT_RESET_VALUE);
	usleep_range(AW_1000_US, AW_1000_US + 100);

	aw_pa->ops.aw_i2c_read(aw_pa->i2c, AW_PID_2055_VERSION_DIFF_REG,
			       &reg_data);
...
static int aw_pid_2113_frcset_check(struct aw_device *aw_dev)
{
	...
	aw_dev->ops.aw_i2c_read(aw_dev->i2c, AW_PID_2113_EFRH3_REG, &reg_val);
	temh = ((uint16_t)reg_val & (~AW_PID_2113_TEMH_MASK));

	aw_dev->ops.aw_i2c_read(aw_dev->i2c, AW_PID_2113_EFRL3_REG, &reg_val);
...
static void aw_pid_2113_reg_force_set(struct aw_device *aw_dev)
{
	...
	if (aw_dev->frcset_en == AW_FRCSET_ENABLE) {
		...
		aw_dev->ops.aw_i2c_write_bits(
			aw_dev->i2c, AW_PID_2113_BSTCTRL3_REG,
			AW_PID_2113_FORCE_PWM_MASK,
			AW_PID_2113_FORCE_PWM_FORCEMINUS_PWM_VALUE);
		/*set BOOST_OS_WIDTH*/
		aw_dev->ops.aw_i2c_write_bits(
			aw_dev->i2c, AW_PID_2113_BSTCTRL5_REG,
		...
		/*set BURST_SSMODE*/
		aw_dev->ops.aw_i2c_write_bits(
			aw_dev->i2c, AW_PID_2113_BSTCTRL8_REG,
...
static void aw_pid_2116_dev_init(struct aw_device *aw_pa)
{
	...
	aw_pa->noise_gate_desc.disable =
		AW_PID_2116_NOISE_GATE_EN_DISABLE_VALUE;
					  ...
					  int32_t *offset)
{
	...
	ret = aw_dev->ops.aw_i2c_read(aw_dev->i2c, AW_PID_2308_EFRH2_REG,
				      &reg_value);
	aw_dev_dbg(aw_dev->dev, "get addr %x val 0x%x", AW_PID_2308_EFRH2_REG,
		   reg_value);
...
static void aw_pid_2308_dev_init(struct aw_device *aw_pa)
{
	...
	/*call aw device init func*/
	memcpy(aw_pa->monitor_name, AW_PID_2308_MONITOR_FILE,
	       strlen(AW_PID_2308_MONITOR_FILE));
...
int aw882xx_init(struct aw_device *aw_pa)
{
	...
	default:
		aw_dev_err(aw_pa->dev, "unsupported chip id 0x%04x",
			   aw_pa->chip_id);
		return -EINVAL;
	}
