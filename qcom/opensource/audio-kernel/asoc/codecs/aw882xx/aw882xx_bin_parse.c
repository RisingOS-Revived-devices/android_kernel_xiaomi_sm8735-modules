					...
					int is_default)
{
	...
	for (i = 0; i < prof_hdr->a_ddt_num; i++) {
		if ((is_default &&
		     ...
		     (aw_dev->channel == cfg_dde[i].dev_index) &&
		     (aw_dev->chip_id == cfg_dde[i].chip_id)) ||
		    (!is_default && (cfg_dde[i].type == AW_DEV_TYPE_ID) &&
		     (aw_dev->i2c->adapter->nr == cfg_dde[i].dev_bus) &&
		     (aw_dev->i2c->addr == cfg_dde[i].dev_addr) &&
		     (aw_dev->chip_id == cfg_dde[i].chip_id))) {
			int j = 0;
			...
			for (j = 0; j < valid_ddt_count; j++) {
				if (cfg_dde[i].dev_profile >
				    cfg_dde[valid_ddt_table[j]].dev_profile)
					continue;

				valid_ddt_count++;
				for (k = 0; k < valid_ddt_count - 1 - j; k++)
					valid_ddt_table[valid_ddt_count - k - 1] =
						valid_ddt_table[valid_ddt_count -
								k - 2];
				valid_ddt_table[j] = i;
				break;
			}
	...
	for (i = 0; i < valid_ddt_count; i++) {
		ret = aw_dev_parse_drv_type_v_1_0_0_0(
			aw_dev, prof_hdr, &cfg_dde[valid_ddt_table[i]],
			&cur_scene_id);
		if (ret < 0)
			return ret;
	}
