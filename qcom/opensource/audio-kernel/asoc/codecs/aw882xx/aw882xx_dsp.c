				...
				int32_t *voltage_offset)
{
	...
	ret = aw_read_msg_from_dsp_v_1_0_0_0(aw_dev, msg_id,
					     (char *)voltage_offset,
					     sizeof(int32_t),
					     AW_DSP_CHANNEL_DEFAULT_NUM);
	if (ret) {
				 ...
				 int32_t voltage_offset)
{
	...
	msg_id = AW_MSG_ID_VOLTAGE_OFFSET;
	ret = aw_write_msg_to_dsp_v_1_0_0_0(aw_dev, msg_id,
					    (char *)&voltage_offset,
					    sizeof(int32_t),
					    AW_DSP_CHANNEL_DEFAULT_NUM);
	if (ret) {
...
int aw882xx_dsp_read_algo_auth_data(struct aw_device *aw_dev, char *data,
				    unsigned int data_len)
{
	int ret = 0;

	ret = aw_read_data_from_dsp(AW_MSG_ID_ALGO_AUTHENTICATION, data,
				    data_len);
	if (ret)
...
int aw882xx_dsp_write_algo_auth_data(struct aw_device *aw_dev, char *data,
				     unsigned int data_len)
{
	int ret = 0;

	ret = aw_write_data_to_dsp(AW_MSG_ID_ALGO_AUTHENTICATION, data,
				   data_len);
	if (ret) {
